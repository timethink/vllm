...
using ByteV2DefaultLayout = vllm::byte_v2::ByteV2PageLayoutV4<>;
...
using ByteV2BN128Layout = vllm::byte_v2::ByteV2PageLayoutV4<ByteV2BN128Policy>;
...
void check_byte_v2_tile_policy(const std::vector<int64_t>& tile_policy) {
  STD_TORCH_CHECK(tile_policy.size() >= 6 && tile_policy.size() <= 14,
                  "ByteV2 tile_policy must contain 6 to 14 values");
...
byte_v2_collect_page_unsafe_stats(const uint8_t* __restrict__ page) {
  ...
  ByteV2PageUnsafeStats stats{0, 0};
#pragma unroll
...
__device__ __forceinline__ int byte_v2_page_is_unsafe(
  ...
  const ByteV2PageUnsafeStats stats = byte_v2_collect_page_unsafe_stats(page);
  return stats.fallback_tiles != 0 || stats.outlier_tiles != 0;
}
...
__device__ __forceinline__ float byte_v2_load_payload_elem(
  ...
  if ((outlier_mask & (uint32_t{1} << codec_tile_idx)) != 0) {
    const int count_offset =
        IsValue ? Layout::v_outlier_count_offset(kv_head, dim_tile, token_tile)
                : Layout::k_outlier_count_offset(kv_head, dim_tile, token_tile);
    const int outlier_count = static_cast<int>(page[count_offset]);
    const int outlier_base_offset =
        IsValue
            ? Layout::v_outlier_payload_offset(kv_head, dim_tile, token_tile)
            : Layout::k_outlier_payload_offset(kv_head, dim_tile, token_tile);
...
byte_v2_make_payload_tile_descriptor(const uint8_t* __restrict__ page,
  ...
  if (use_masks) {
    ...
    if (desc.outlier_hit) {
      const int count_offset =
          IsValue ? Layout::v_outlier_count_offset(kv_head, dim_tile, 0)
                  : Layout::k_outlier_count_offset(kv_head, dim_tile, 0);
      desc.outlier_count = static_cast<int>(page[count_offset]);
      desc.outlier_payload_offset =
          IsValue ? Layout::v_outlier_payload_offset(kv_head, dim_tile, 0)
                  : Layout::k_outlier_payload_offset(kv_head, dim_tile, 0);
    }
  }
...
byte_v2_load_payload_elem_hex_from_safe_tile_descriptor_fixed_dim_bits(
  ...
  bits0 = byte_v2_make_payload_bits_from_low_and_codes<0, 0>(low0, packed_codes,
                                                             desc.base);
  bits1 = byte_v2_make_payload_bits_from_low_and_codes<1, 1>(low0, packed_codes,
                                                             desc.base);
  bits2 = byte_v2_make_payload_bits_from_low_and_codes<2, 2>(low0, packed_codes,
                                                             desc.base);
  bits3 = byte_v2_make_payload_bits_from_low_and_codes<3, 3>(low0, packed_codes,
                                                             desc.base);
  bits4 = byte_v2_make_payload_bits_from_low_and_codes<4, 4>(low0, packed_codes,
                                                             desc.base);
  bits5 = byte_v2_make_payload_bits_from_low_and_codes<5, 5>(low0, packed_codes,
                                                             desc.base);
  bits6 = byte_v2_make_payload_bits_from_low_and_codes<6, 6>(low0, packed_codes,
                                                             desc.base);
  bits7 = byte_v2_make_payload_bits_from_low_and_codes<7, 7>(low0, packed_codes,
                                                             desc.base);
  bits8 = byte_v2_make_payload_bits_from_low_and_codes<0, 8>(low1, packed_codes,
                                                             desc.base);
  bits9 = byte_v2_make_payload_bits_from_low_and_codes<1, 9>(low1, packed_codes,
                                                             desc.base);
...
byte_v2_paged_decode_attention_split_k_gqa4_fa2_like_no_fallback_no_outlier_kernel(
  ...
  static_assert(!UseFa2Direct || QGroupTile <= 16);
...
#pragma unroll
  ...
  if constexpr (UseFa2Direct) {
    __shared__ __align__(16) typename CuteDirectQkTraits::Element
        shared_q_cute[CuteDirectQkTraits::kQStorageElems];
    __shared__ __align__(16) typename CuteDirectQkTraits::Element
...
#pragma unroll 1
    for (int elem = threadIdx.x; elem < kDirectBlockM * Policy::HeadDim;
         elem += NumThreads) {
      const int row = elem / Policy::HeadDim;
      const int col = elem - row * Policy::HeadDim;
      uint16_t bits = 0;
      if (row < valid_q_rows) {
        const int head_idx = kv_head_idx * kQHeadsPerKv + q_group_base + row;
...
#pragma unroll 1
          for (int row_rel = threadIdx.x / kDimHexes; row_rel < staged_rows;
            ...
            byte_v2_store_16_bf16_bits_to_smem(
                &shared_k_cute[0], typename CuteDirectQkTraits::SmemLayoutKV{},
                tile_offset, q_dim, bits0, bits1, bits2, bits3, bits4, bits5,
                bits6, bits7, bits8, bits9, bits10, bits11, bits12, bits13,
                bits14, bits15);
            byte_v2_load_payload_elem_hex_from_safe_tile_descriptor_fixed_dim_bits<
                Layout>(v_desc0, row, dim_in_tile, bits0, bits1, bits2, bits3,
                        ...
                        bits11, bits12, bits13, bits14, bits15);
            byte_v2_store_16_bf16_bits_to_smem(
                &shared_v_full[0], typename CuteDirectPvTraits::SmemLayoutKV{},
                tile_offset, q_dim, bits0, bits1, bits2, bits3, bits4, bits5,
                bits6, bits7, bits8, bits9, bits10, bits11, bits12, bits13,
                bits14, bits15);
          }
...
#pragma unroll
        for (int elem = 0; elem < cute::size(acc_s); ++elem) {
          ...
          if (row < valid_q_rows) {
            const float score =
                col < direct_tile_len ? acc_s(elem) * scale : -FLT_MAX;
...
#pragma unroll
        for (int q_group = 0; q_group < kQPerKv; ++q_group) {
          row_l[q_group] = 0.0f;
          new_m[q_group] = fmaxf(softmax_m[q_group], row_m[q_group]);
          alpha[q_group] = softmax_l[q_group] > 0.0f
                               ? __expf(softmax_m[q_group] - new_m[q_group])
                               : 0.0f;
        }
#pragma unroll
        for (int elem = 0; elem < cute::size(acc_s); ++elem) {
          ...
          const int col = cute::get<1>(tCcS(elem));
          float prob = 0.0f;
          if (row < valid_q_rows && col < direct_tile_len &&
              new_m[row] != -FLT_MAX) {
            prob = __expf(acc_s(elem) * scale - new_m[row]);
            row_l[row] += prob;
          }
          acc_s(elem) = prob;
        }
...
#pragma unroll
        for (int elem = 0; elem < cute::size(acc_o); ++elem) {
          ...
          if (q_group < valid_q_rows) {
            acc_o(elem) = acc_o(elem) * alpha[q_group] + acc_o_tile(elem);
          } else {
            acc_o(elem) = 0.0f;
          }
        }
#pragma unroll
        for (int q_group = 0; q_group < kQPerKv; ++q_group) {
          softmax_l[q_group] =
              softmax_l[q_group] * alpha[q_group] + row_l[q_group];
          softmax_m[q_group] = new_m[q_group];
        }
      }
      __syncthreads();
    }
...
#pragma unroll
      for (int elem = 0; elem < cute::size(acc_o); ++elem) {
        ...
        if (q_group < valid_q_rows && col < Policy::HeadDimV) {
          const float denom = softmax_l[q_group];
      ...
      if (kv_side == 0) {
        ...
        if (fallback) {
          ...
          atomicOr(
              ...
              1u << tile_idx);
          page[ByteV2DefaultLayout::k_outlier_count_offset(head_idx, dim_tile,
                                                           token_tile)] =
              static_cast<uint8_t>(outlier_count);
        }
      } else {
        ...
        if (fallback) {
          ...
          atomicOr(
              ...
              1u << tile_idx);
          page[ByteV2DefaultLayout::v_outlier_count_offset(head_idx, dim_tile,
                                                           token_tile)] =
              static_cast<uint8_t>(outlier_count);
        }
      }
            ...
            const int entry_offset =
                kv_side == 0
                    ? ByteV2DefaultLayout::k_outlier_payload_offset(
                          head_idx, dim_tile, token_tile, overlay_entry_idx)
...
__global__ void byte_v2_update_cache_unsafe_flags_kernel(
  ...
  for (int64_t token_idx = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    const uint8_t* __restrict__ page =
        kv_cache + physical_block * kv_cache_stride_block;
    page_unsafe_flags[physical_block] = byte_v2_page_is_unsafe(page);
  }
}
  ...
  if (threadIdx.x == 0) {
    ...
    const int base_offset = kv_side == 0 ? ByteV2DefaultLayout::k_base_offset(
                                         ...
                                         : ByteV2DefaultLayout::v_base_offset(
                                               head_idx, dim_tile, token_tile);
    const int outlier_count_offset =
        kv_side == 0 ? ByteV2DefaultLayout::k_outlier_count_offset(
                           head_idx, dim_tile, token_tile)
                     : ByteV2DefaultLayout::v_outlier_count_offset(
                           head_idx, dim_tile, token_tile);
    ...
    if (update_row == 0) {
      ...
      outlier_count = (previous_outlier_mask & bit) != 0
                          ? static_cast<int>(page[outlier_count_offset])
                          : 0;
    }
      ...
      if (outlier_count < ByteV2DefaultLayout::OutlierEntriesPerTileValue) {
        ...
        const int entry_offset =
            kv_side == 0 ? ByteV2DefaultLayout::k_outlier_payload_offset(
                               head_idx, dim_tile, token_tile, outlier_count)
                         : ByteV2DefaultLayout::v_outlier_payload_offset(
                               head_idx, dim_tile, token_tile, outlier_count);
        byte_v2_store_u16_bytes(page, entry_offset, entry);
      }
...
void launch_byte_v2_paged_decode_attention_split_k_gqa4_fa2_like_no_fallback_no_outlier(
  ...
  constexpr int kGroupsPerKv = (QHeadsPerKv + QGroupTile - 1) / QGroupTile;
  static_assert(kThreads == 128);
  static_assert(Layout::TilePolicy::ComputeBlockN == 64);
  static_assert(QHeadsPerKv > 0);
  static_assert(QGroupTile > 0);
  static_assert(QGroupTile <= QHeadsPerKv);
  ...
  byte_v2_paged_decode_attention_split_k_gqa4_fa2_like_no_fallback_no_outlier_kernel<
      ...
      UseFa2Direct, QHeadsPerKv, QGroupTile, kThreads>
      <<<split_grid, block, 0, stream>>>(
          reinterpret_cast<float*>(tmp_out.mutable_data_ptr()),
          reinterpret_cast<float*>(exp_sums.mutable_data_ptr()),
          max_logits.numel() == 0
...
void byte_v2_reshape_and_cache(torch::stable::Tensor& key,
  ...
  STD_TORCH_CHECK(kv_cache.size(1) >= ByteV2DefaultLayout::PageSizeBytes,
                  "kv_cache page size is smaller than ByteV2 V4 layout");
  STD_TORCH_CHECK(kv_cache.stride(0) >= ByteV2DefaultLayout::PageSizeBytes,
                  "kv_cache block stride is smaller than ByteV2 V4 layout");
  ...
  const dim3 grid(
      ...
      1);
  byte_v2_reshape_and_cache_block_direct_kernel<<<grid, kThreads, 0, stream>>>(
...
void byte_v2_update_cache_single_token(torch::stable::Tensor& key,
  ...
  byte_v2_update_cache_single_token_kernel<<<grid, kThreads, 0, stream>>>(
      ...
      reinterpret_cast<const uint16_t*>(value.const_data_ptr()),
      reinterpret_cast<uint8_t*>(kv_cache.mutable_data_ptr()),
      slot_mapping.const_data_ptr<int64_t>(), key.size(1), key.stride(0),
      key.stride(1), key.stride(2), value.stride(0), value.stride(1),
      value.stride(2), kv_cache.size(0), kv_cache.stride(0));
  const cudaError_t err = cudaGetLastError();
  STD_TORCH_CHECK(err == cudaSuccess,
...
void byte_v2_commit_raw_staging_to_cache(
  ...
  const int clear_blocks =
      static_cast<int>((clear_work_items + kClearThreads - 1) / kClearThreads);
  byte_v2_clear_page_metadata_from_staging_kernel<<<clear_blocks, kClearThreads,
                                                    0, stream>>>(
      reinterpret_cast<uint8_t*>(kv_cache.mutable_data_ptr()),
      staging_to_physical_block.const_data_ptr<int32_t>(), num_staging_slots,
      kv_cache.stride(0));

  byte_v2_commit_raw_staging_to_cache_kernel<<<grid, kThreads, 0, stream>>>(
