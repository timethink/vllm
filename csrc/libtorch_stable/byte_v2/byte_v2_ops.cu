...
constexpr int64_t kByteV2PageParallelPrepareMinTokens = 256;
constexpr int32_t kByteV2PageParallelPrepareFallback = 2;

int64_t byte_v2_cooperative_multi_token_writer_limit() {
  static const int64_t limit = []() -> int64_t {
    constexpr const char* kEnvName = "BYTE_V2_HYBRID_COOPERATIVE_WRITER_LIMIT";
    const char* value = std::getenv(kEnvName);
    // The expanded cooperative route is validated for production cached-tail
    // waves. Keep 32 as a same-binary rollback and 16 for the older dispatch
    // boundary experiment.
    if (value == nullptr || std::strcmp(value, "144") == 0) {
      return kByteV2MaxCooperativeMultiTokenWriterTokens;
    }
    if (std::strcmp(value, "32") == 0) {
      return kByteV2DefaultCooperativeMultiTokenWriterTokens;
    }
    if (std::strcmp(value, "16") == 0) {
      return ByteV2DefaultPolicy::AllocBlockTokens;
    }
    STD_TORCH_CHECK(false, kEnvName, " must be one of 16, 32, or 144, got ",
                    value);
    return kByteV2DefaultCooperativeMultiTokenWriterTokens;
  }();
  return limit;
...
bool byte_v2_hybrid_full_wave_writer_enabled() {
  // The validated full-wave path is the default. Keep the selector as a
  // fail-closed rollback and same-binary attribution switch.
...
bool byte_v2_page_parallel_raw_staging_prepare_enabled() {
  // Keep the serialized implementation in the same binary as a rollback and
  // attribution control. The page-parallel route only publishes descriptors
  // after every 16-token group passes validation; arbitrary mappings use the
  // original serialized implementation.
  static const bool enabled = []() -> bool {
    constexpr const char* kEnvName =
        "BYTE_V2_RAW_STAGING_PAGE_PARALLEL_PREPARE";
    const char* value = std::getenv(kEnvName);
    if (value == nullptr || std::strcmp(value, "1") == 0) {
      return true;
    }
...
struct ByteV2StaticW8Bases {
  // Host-selected scalars only affect the writer; the V6 reader ABI is fixed.
  bool enabled;
  int k;
  int v;
};

ByteV2StaticW8Bases byte_v2_static_w8_bases(int64_t k, int64_t v) {
  const bool dynamic = k == -1 && v == -1;
  const bool valid_static = k >= 0 && k <= 120 && v >= 0 && v <= 120;
  STD_TORCH_CHECK(
      dynamic || valid_static,
      "ByteV2 Static-W8 bases must both be -1 (Dynamic-W8) or integers in "
      "[0, 120], got K=",
      k, ", V=", v);
  return {valid_static, static_cast<int>(k), static_cast<int>(v)};
}
...
__device__ __forceinline__ void byte_v2_store_aligned_u16(
    uint8_t* __restrict__ base, int64_t offset, uint16_t bits) {
  *reinterpret_cast<uint16_t*>(base + offset) = bits;
}

template <bool Aligned>
__device__ __forceinline__ uint16_t
byte_v2_load_raw_staging_u16(const uint8_t* __restrict__ base, int64_t offset) {
  if constexpr (Aligned) {
    return byte_v2_load_aligned_u16(base, offset);
  }
  return byte_v2_load_u16_bytes(base, offset);
}

template <bool Aligned>
__device__ __forceinline__ void byte_v2_store_raw_staging_u16(
    uint8_t* __restrict__ base, int64_t offset, uint16_t bits) {
  if constexpr (Aligned) {
    byte_v2_store_aligned_u16(base, offset, bits);
  } else {
    byte_v2_store_u16_bytes(base, offset, bits);
  }
}
...
template <typename Layout>
__device__ __forceinline__ int byte_v2_try_allocate_outlier_segment(
  ...
  if constexpr (!Layout::PagePooledOutliersValue) {
    ...
    auto* __restrict__ pool_used =
        reinterpret_cast<unsigned int*>(page + Layout::OutlierPoolUsedOffset);
    unsigned int observed = atomicAdd(pool_used, 0u);
    while (true) {
      if (observed > Layout::OutlierPoolEntriesValue ||
          static_cast<unsigned int>(capacity) >
              Layout::OutlierPoolEntriesValue - observed) {
      ...
      const unsigned int desired =
          observed + static_cast<unsigned int>(capacity);
      const unsigned int prior = atomicCAS(pool_used, observed, desired);
      if (prior == observed) {
...
__device__ __forceinline__ int byte_v2_allocate_outlier_segment(
  ...
  const int pool_index =
      byte_v2_try_allocate_outlier_segment<Layout>(page, count);
  if (pool_index < 0) {
    __trap();
  }
  return pool_index;
...
__device__ __forceinline__ void byte_v2_prepare_raw_staging_serial_impl(
...
__global__ void byte_v2_prepare_raw_staging_kernel(
  ...
  byte_v2_prepare_raw_staging_serial_impl(
      slot_mapping, block_to_staging_slot, staging_to_physical_block,
      valid_rows, next_staging_slot, overflow, num_tokens, num_physical_blocks,
      num_staging_slots);
}

__global__ void byte_v2_validate_page_parallel_raw_staging_kernel(
    const int64_t* __restrict__ slot_mapping,
    int32_t* __restrict__ block_to_staging_slot,
    int32_t* __restrict__ staging_to_physical_block,
    int32_t* __restrict__ valid_rows,
    const int32_t* __restrict__ next_staging_slot,
    int32_t* __restrict__ overflow, int64_t num_tokens,
    int64_t num_physical_blocks, int64_t num_page_groups) {
  constexpr int kBlockSize = ByteV2DefaultPolicy::AllocBlockTokens;
  constexpr int kWarpSize = 32;
  ...
  for (int64_t group = first_group; group < num_page_groups;
       group += group_stride) {
    const int32_t initial_next =
        __shfl_sync(kFullWarpMask, lane == 0 ? next_staging_slot[0] : 0, 0);
    const int32_t prepare_mode =
        __shfl_sync(kFullWarpMask, lane == 0 ? atomicAdd(overflow, 0) : 0, 0);
    if (initial_next != 0 || prepare_mode != 0) {
      if (lane == 0) {
        atomicExch(overflow, kByteV2PageParallelPrepareFallback);
      }
      continue;
    }

    const int64_t first_token = group * kBlockSize;
    const int token_count = static_cast<int>(
        min(static_cast<int64_t>(kBlockSize), num_tokens - first_token));
    int64_t slot_idx = -1;
    if (lane < token_count) {
      slot_idx = slot_mapping[first_token + lane];
    }
    ...
    if (slot_idx >= 0) {
      ...
      if (physical_block64 >= 0 && physical_block64 < num_physical_blocks &&
          physical_block64 <= std::numeric_limits<int32_t>::max()) {
        physical_block = static_cast<int32_t>(physical_block64);
      }
    }
    const int32_t first_physical =
        __shfl_sync(kFullWarpMask, physical_block, 0);
    const bool lane_valid =
        lane >= token_count ||
        (physical_block >= 0 && physical_block == first_physical);
    const bool group_valid = __all_sync(kFullWarpMask, lane_valid);

    int32_t row_plus_one = 0;
    if (lane < token_count && lane_valid) {
      row_plus_one = static_cast<int32_t>(slot_idx % kBlockSize) + 1;
    }
    for (int offset = kWarpSize / 2; offset > 0; offset /= 2) {
      row_plus_one = max(row_plus_one,
                         __shfl_down_sync(kFullWarpMask, row_plus_one, offset));
    }

    if (lane != 0) {
      continue;
    }
    if (!group_valid || first_physical < 0 || token_count <= 0 ||
        staging_to_physical_block[group] != -1 || valid_rows[group] != 0) {
      atomicExch(overflow, kByteV2PageParallelPrepareFallback);
      continue;
    }

    const int32_t claim = -static_cast<int32_t>(group) - 2;
    const int32_t prior =
        atomicCAS(block_to_staging_slot + first_physical, -1, claim);
    if (prior != -1) {
      atomicExch(overflow, kByteV2PageParallelPrepareFallback);
      continue;
    }
    staging_to_physical_block[group] = first_physical;
    valid_rows[group] = row_plus_one;
  }
}

__global__ void byte_v2_finalize_page_parallel_raw_staging_kernel(
    int32_t* __restrict__ block_to_staging_slot,
    int32_t* __restrict__ staging_to_physical_block,
    int32_t* __restrict__ valid_rows, int32_t* __restrict__ next_staging_slot,
    int32_t* __restrict__ overflow, int64_t num_page_groups,
    int64_t num_physical_blocks) {
  const int32_t mode = atomicAdd(overflow, 0);
  for (int64_t group =
           static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
       group < num_page_groups;
       group += static_cast<int64_t>(blockDim.x) * gridDim.x) {
    const int32_t physical_block = staging_to_physical_block[group];
    const int32_t claim = -static_cast<int32_t>(group) - 2;
    if (mode == kByteV2PageParallelPrepareFallback) {
      if (physical_block >= 0 && physical_block < num_physical_blocks &&
          atomicCAS(block_to_staging_slot + physical_block, claim, -1) ==
              claim) {
        staging_to_physical_block[group] = -1;
        valid_rows[group] = 0;
      }
      continue;
    }
    ...
    // All recoverable validation happens before this kernel. A failure here
    // means the same-stream workspace lifecycle was violated, so fail closed
    // instead of exposing a partially published map to the serial fallback.
    if (physical_block < 0 || physical_block >= num_physical_blocks ||
        valid_rows[group] <= 0 ||
        valid_rows[group] > ByteV2DefaultPolicy::AllocBlockTokens ||
        atomicCAS(block_to_staging_slot + physical_block, claim,
                  static_cast<int32_t>(group)) != claim) {
      atomicExch(overflow, 1);
      __trap();
    }
  }

  if (blockIdx.x == 0 && threadIdx.x == 0 && mode == 0) {
    next_staging_slot[0] = static_cast<int32_t>(num_page_groups);
  }
}

__global__ void byte_v2_prepare_raw_staging_serial_fallback_kernel(
    const int64_t* __restrict__ slot_mapping,
    int32_t* __restrict__ block_to_staging_slot,
    int32_t* __restrict__ staging_to_physical_block,
    int32_t* __restrict__ valid_rows, int32_t* __restrict__ next_staging_slot,
    ...
    int64_t num_physical_blocks, int64_t num_staging_slots) {
  if (blockIdx.x != 0 || threadIdx.x != 0 ||
      *overflow != kByteV2PageParallelPrepareFallback) {
    return;
  }
  *overflow = 0;
  byte_v2_prepare_raw_staging_serial_impl(
...
__global__ void byte_v2_hydrate_raw_staging_from_cache_kernel(
    ...
    const uint8_t* __restrict__ persistent_raw_staging,
    const int32_t* __restrict__ page_to_raw_slot, int64_t persistent_raw_slots,
    ...
    if (page_to_raw_slot != nullptr) {
      ...
      if (raw_slot >= 0) {
        ...
        const uint8_t* __restrict__ raw_page =
            ...
            static_cast<int64_t>(raw_slot) * persistent_raw_stride_slot;
        byte_v2_store_u16_bytes(
            staging, staging_offset,
            byte_v2_load_u16_bytes(raw_page, staging_offset));
        continue;
      }
    }
    ...
    const float value =
        kv_side == 0
            ? byte_v2_load_payload_elem<ByteV2DefaultLayout, false>(
                  ...
                  ByteV2DefaultLayout::KPayloadBaseBytes +
                      head_idx *
                          ByteV2DefaultLayout::AlignedKPayloadBytesPerKvHead,
                  head_idx, row, dim)
            : byte_v2_load_payload_elem<ByteV2DefaultLayout, true>(
                  page,
                  ByteV2DefaultLayout::VPayloadBaseBytes +
                      head_idx *
                          ByteV2DefaultLayout::AlignedVPayloadBytesPerKvHead,
                  head_idx, row, dim);
    byte_v2_store_u16_bytes(staging, staging_offset,
...
// Experimental B1/Q1 mutable-tail writer. Partial pages remain authoritative
// in the persistent raw sidecar, so the common mapped-page path writes only
// the newly appended 4 KiB K/V row. A newly mapped page materializes its valid
// compact prefix exactly once and publishes page_to_raw_slot only after every
// raw store is globally visible. The full-page copy into raw_staging is needed
// only on row 15 by the unchanged compact commit kernel launched next. This
// protocol relies on update and attention executing on the same CUDA stream;
// it does not provide cross-stream cache synchronization.
__global__ void byte_v2_append_persistent_raw_tail_q1_kernel(
    const uint16_t* __restrict__ key, const uint16_t* __restrict__ value,
    uint8_t* __restrict__ raw_staging, uint8_t* __restrict__ kv_cache,
    uint8_t* __restrict__ persistent_raw_staging,
    const int64_t* __restrict__ slot_mapping,
    const int32_t* __restrict__ block_to_staging_slot,
    int32_t* __restrict__ staging_to_physical_block,
    int32_t* __restrict__ valid_rows, int32_t* __restrict__ next_staging_slot,
    ...
    int32_t* __restrict__ page_unsafe_flags, int64_t key_stride_head,
    int64_t key_stride_dim, int64_t value_stride_head, int64_t value_stride_dim,
    int64_t num_physical_blocks, int64_t kv_cache_stride_block,
    int64_t persistent_raw_slots, int64_t persistent_raw_stride_slot) {
  constexpr int kBlockSize = ByteV2DefaultPolicy::AllocBlockTokens;
  constexpr int kHeadDim = ByteV2DefaultPolicy::HeadDim;
  constexpr int kHeadDimV = ByteV2DefaultPolicy::HeadDimV;
  constexpr int kNumKvHeads = ByteV2DefaultLayout::NumKvHeadsValue;
  ...
  __shared__ int32_t shared_target_row;
  __shared__ int32_t shared_raw_slot;
  __shared__ int32_t shared_new_raw_slot;
  ...
  if (threadIdx.x == 0) {
    ...
    shared_target_row = -1;
    shared_raw_slot = -1;
    shared_new_raw_slot = 0;
    shared_action = 0;

    const int64_t slot_idx = slot_mapping[0];
    if (slot_idx >= 0) {
      const int64_t physical_block = slot_idx / kBlockSize;
      const int32_t target_row = static_cast<int32_t>(slot_idx % kBlockSize);
      if (physical_block < 0 || physical_block >= num_physical_blocks ||
          target_row < 0 || target_row >= kBlockSize ||
          atomicAdd(raw_pool_overflow, 0) != 0 ||
          atomicAdd(staging_overflow, 0) != 0 || next_staging_slot[0] != 0 ||
          staging_to_physical_block[0] != -1 || valid_rows[0] != 0 ||
          block_to_staging_slot[physical_block] != -1) {
        atomicExch(raw_pool_overflow, 1);
        atomicExch(staging_overflow, 1);
        shared_action = -1;
      } else {
        int32_t raw_slot = page_to_raw_slot[physical_block];
        if (raw_slot < -1 || raw_slot >= persistent_raw_slots) {
          atomicExch(raw_pool_overflow, 1);
          atomicExch(staging_overflow, 1);
          shared_action = -1;
        } else if (raw_slot == -1) {
          // A compact fallback tile or exhausted outlier pool is not an exact
          // hydration source. Such a page must already own a persistent raw
          // slot, so fail closed instead of silently materializing lossy BF16.
          const uint8_t* __restrict__ compact_page =
              kv_cache + physical_block * kv_cache_stride_block;
          const uint32_t compact_pool_overflow = byte_v2_load_u32_bytes(
              compact_page, ByteV2DefaultLayout::OutlierPoolOverflowOffset);
          const ByteV2PageUnsafeStats compact_stats =
              byte_v2_collect_page_unsafe_stats(compact_page);
          if (target_row > 0 && (compact_pool_overflow != 0 ||
                                 compact_stats.fallback_tiles != 0)) {
            atomicExch(raw_pool_overflow, 1);
            atomicExch(staging_overflow, 1);
            shared_action = -1;
          }
          int32_t observed = atomicAdd(free_raw_slot_count, 0);
          while (shared_action == 0 && observed > 0) {
            ...
            if (prior == observed) {
              raw_slot = free_raw_slots[observed - 1];
              break;
            }
            observed = prior;
          }
          if (raw_slot < 0 || raw_slot >= persistent_raw_slots) {
            ...
            shared_new_raw_slot = 1;
          }
        }
        if (shared_action == 0) {
          shared_physical_block = physical_block;
          shared_target_row = target_row;
          shared_raw_slot = raw_slot;
          shared_action = 1;
        }
      }
  ...
  const int64_t physical_block = shared_physical_block;
  const int target_row = shared_target_row;
  const int32_t raw_slot = shared_raw_slot;
  uint8_t* __restrict__ raw_page =
      persistent_raw_staging +
      static_cast<int64_t>(raw_slot) * persistent_raw_stride_slot;
  const uint8_t* __restrict__ compact_page =
      kv_cache + physical_block * kv_cache_stride_block;

  if (shared_new_raw_slot != 0) {
    const int64_t work_items =
        static_cast<int64_t>(2) * kNumKvHeads * (target_row + 1) * kHeadDim;
    for (int64_t work_idx = threadIdx.x; work_idx < work_items;
         work_idx += blockDim.x) {
      int64_t tmp = work_idx;
      const int dim = tmp % kHeadDim;
      tmp /= kHeadDim;
      const int row = tmp % (target_row + 1);
      tmp /= target_row + 1;
      const int head_idx = tmp % kNumKvHeads;
      tmp /= kNumKvHeads;
      const int kv_side = tmp & 1;
      ...
      uint16_t bits;
      if (row == target_row) {
        const uint16_t* __restrict__ source = kv_side == 0 ? key : value;
        const int64_t stride_head =
            kv_side == 0 ? key_stride_head : value_stride_head;
        const int64_t stride_dim =
            kv_side == 0 ? key_stride_dim : value_stride_dim;
        bits = source[head_idx * stride_head + dim * stride_dim];
      } else {
        const float decoded =
            kv_side == 0
                ? byte_v2_load_payload_elem<ByteV2DefaultLayout, false>(
      ...
      const int64_t raw_offset =
          kv_side == 0
              ? ByteV2DefaultRawStagingLayout::key_offset(head_idx, row, dim)
              : ByteV2DefaultRawStagingLayout::value_offset(head_idx, row, dim);
      byte_v2_store_u16_bytes(raw_page, raw_offset, bits);
    }

    // Every copying thread orders its raw-page stores before thread 0 publishes
    // the map. Attention runs later on the same stream and can then treat the
    // sidecar as authoritative without observing a partially hydrated prefix.
    __threadfence();
    __syncthreads();
    if (threadIdx.x == 0) {
      const int32_t prior =
          atomicCAS(page_to_raw_slot + physical_block, -1, raw_slot);
      if (prior != -1) {
        atomicExch(raw_pool_overflow, 1);
        atomicExch(staging_overflow, 1);
        shared_action = -1;
    ...
    // The mapped-page fast path touches only the new row: 2 * 8 * 128 BF16
    // values, or exactly 4 KiB for the supported layout.
    constexpr int kElemsPerVector = sizeof(uint4) / sizeof(uint16_t);
    static_assert(kHeadDim % kElemsPerVector == 0);
    static_assert(kHeadDimV == kHeadDim);
    constexpr int kDimVectors = kHeadDim / kElemsPerVector;
    constexpr int64_t kRowVectorWorkItems =
        static_cast<int64_t>(2) * kNumKvHeads * kDimVectors;
    const bool vector_copy =
        key_stride_dim == 1 && value_stride_dim == 1 &&
        key_stride_head % kElemsPerVector == 0 &&
        value_stride_head % kElemsPerVector == 0 &&
        reinterpret_cast<uintptr_t>(key) % alignof(uint4) == 0 &&
        reinterpret_cast<uintptr_t>(value) % alignof(uint4) == 0;
    if (vector_copy) {
      for (int64_t vector_idx = threadIdx.x; vector_idx < kRowVectorWorkItems;
           vector_idx += blockDim.x) {
        int64_t tmp = vector_idx;
        const int dim = static_cast<int>(tmp % kDimVectors) * kElemsPerVector;
        tmp /= kDimVectors;
        const int head_idx = static_cast<int>(tmp % kNumKvHeads);
        tmp /= kNumKvHeads;
        const int kv_side = static_cast<int>(tmp);
        const uint16_t* __restrict__ source = kv_side == 0 ? key : value;
        const int64_t stride_head =
            kv_side == 0 ? key_stride_head : value_stride_head;
        const int64_t source_offset = head_idx * stride_head + dim;
        const int64_t raw_offset =
            kv_side == 0 ? ByteV2DefaultRawStagingLayout::key_offset(
                               head_idx, target_row, dim)
                         : ByteV2DefaultRawStagingLayout::value_offset(
                               head_idx, target_row, dim);
        *reinterpret_cast<uint4*>(raw_page + raw_offset) =
            *reinterpret_cast<const uint4*>(source + source_offset);
      }
    } else {
      constexpr int64_t kRowWorkItems =
          static_cast<int64_t>(2) * kNumKvHeads * kHeadDim;
      for (int64_t work_idx = threadIdx.x; work_idx < kRowWorkItems;
           work_idx += blockDim.x) {
        int64_t tmp = work_idx;
        const int dim = tmp % kHeadDim;
        tmp /= kHeadDim;
        ...
        const int64_t stride_dim =
            kv_side == 0 ? key_stride_dim : value_stride_dim;
        const uint16_t bits = source[head_idx * stride_head + dim * stride_dim];
        const int64_t raw_offset =
            kv_side == 0 ? ByteV2DefaultRawStagingLayout::key_offset(
                               head_idx, target_row, dim)
                         : ByteV2DefaultRawStagingLayout::value_offset(
    ...
    // The raw map is already published. A same-stream consumer cannot begin
    // until this kernel completes, so partial rows need no second publication
    // fence. The row-15 in-kernel raw-page copy does require a CTA barrier.
    if (target_row == kBlockSize - 1) {
      __syncthreads();
    }
  }

  if (page_unsafe_flags != nullptr && threadIdx.x == 0) {
    // The compact copy is stale while the sidecar is authoritative. Keep every
    // compact-only fast path fail-closed until full-page finalization refreshes
    // this flag from the newly sealed page.
    page_unsafe_flags[physical_block] =
        kByteV2PageUnsafeAny | kByteV2PageUnsafeK | kByteV2PageUnsafeV;
  }
  ...
  const auto* __restrict__ source_vectors =
      reinterpret_cast<const uint4*>(raw_page);
  auto* __restrict__ staging_vectors = reinterpret_cast<uint4*>(raw_staging);
  for (int64_t vector_idx = threadIdx.x; vector_idx < kVectorsPerRawPage;
       vector_idx += blockDim.x) {
    staging_vectors[vector_idx] = source_vectors[vector_idx];
  }

  uint8_t* __restrict__ compact_page_mutable =
      kv_cache + physical_block * kv_cache_stride_block;
  for (int metadata_byte = threadIdx.x;
       metadata_byte < ByteV2DefaultLayout::AlignedMetadataBytes;
       metadata_byte += blockDim.x) {
    compact_page_mutable[metadata_byte] = 0;
  }
  __syncthreads();
  if (threadIdx.x == 0) {
    // Publishing this transient descriptor activates exactly one full-page
    // compact commit in the fixed launch that follows. Partial pages leave the
    // descriptor clean, so all commit CTAs return immediately.
    staging_to_physical_block[0] = static_cast<int32_t>(physical_block);
    valid_rows[0] = kBlockSize;
  }
}

__global__ void byte_v2_prepare_small_multi_token_hybrid_staging_kernel(
    const int64_t* __restrict__ slot_mapping,
    int32_t* __restrict__ block_to_staging_slot,
    int32_t* __restrict__ staging_to_physical_block,
    int32_t* __restrict__ valid_rows, int32_t* __restrict__ next_staging_slot,
  ...
  __shared__ uint32_t candidate_row_masks[kMaxTokens];
  ...
  int32_t invalid_state =
      atomicAdd(staging_overflow, 0) != 0 ||
              ...
              stage_ctas_per_page <= 0
          ? 1
          : 0;
  if (next_staging_slot[0] != 0) {
    invalid_state = 1;
  }
  for (int64_t staging_slot = 0; staging_slot < num_staging_slots;
       ++staging_slot) {
  ...
  // Validate and collect every unique page before mutating allocator state.
  // This makes malformed inputs fail closed without partially publishing a
  // staging map.
  ...
  for (int64_t token_idx = 0; token_idx < num_tokens; ++token_idx) {
    ...
    const int32_t physical_block = static_cast<int32_t>(physical_block64);
    const int32_t row = static_cast<int32_t>(slot_idx % kBlockSize);
    int32_t candidate_idx = -1;
    for (int32_t idx = 0; idx < candidate_count; ++idx) {
      if (candidate_pages[idx] == physical_block) {
        candidate_idx = idx;
        break;
      }
    }
    if (candidate_idx < 0) {
      ...
      candidate_pages[candidate_idx] = physical_block;
      candidate_row_masks[candidate_idx] = 0;
      candidate_valid_rows[candidate_idx] = 0;
    }
    const uint32_t row_bit = uint32_t{1} << row;
    if ((candidate_row_masks[candidate_idx] & row_bit) != 0) {
      invalid_state = 1;
      continue;
    }
    candidate_row_masks[candidate_idx] |= row_bit;
    candidate_valid_rows[candidate_idx] =
        max(candidate_valid_rows[candidate_idx], row + 1);
  }
  if (invalid_state != 0) {
    atomicExch(staging_overflow, 1);
    atomicExch(raw_pool_overflow, 1);
...
__global__ void byte_v2_hydrate_append_small_multi_token_hybrid_staging_kernel(
    ...
    int64_t value_stride_dim, int64_t num_physical_blocks,
    int64_t num_staging_slots, int64_t raw_staging_stride_slot,
  ...
  constexpr int kNumKvHeads = ByteV2DefaultLayout::NumKvHeadsValue;
  constexpr int kCompletionShift = 5;
  constexpr int32_t kCompletionUnit = 1 << kCompletionShift;
  constexpr int32_t kRowsMask = kCompletionUnit - 1;

  __shared__ int32_t token_for_row[kBlockSize];
  __shared__ int32_t shared_raw_slot;
  __shared__ int32_t shared_row_state;
  __shared__ int32_t shared_invalid_state;
  __shared__ int32_t shared_is_last;

  const int32_t staging_slot = static_cast<int32_t>(blockIdx.y);
  if (staging_slot < 0 || staging_slot >= num_staging_slots) {
    return;
  }
  const int32_t physical_block = staging_to_physical_block[staging_slot];
  ...
  if (threadIdx.x == 0) {
    shared_row_state = atomicAdd(valid_rows + staging_slot, 0);
    const int32_t rows = shared_row_state & kRowsMask;
    shared_invalid_state =
        physical_block >= num_physical_blocks || rows <= 0 ||
                rows > kBlockSize ||
                (shared_row_state >> kCompletionShift) <= 0 ||
                (shared_row_state >> kCompletionShift) >
                    static_cast<int32_t>(gridDim.x) ||
                atomicAdd(staging_overflow, 0) != 0 ||
                atomicAdd(raw_pool_overflow, 0) != 0 ||
                block_to_staging_slot[physical_block] != -2
            ? 1
            : 0;
    shared_raw_slot =
        shared_invalid_state == 0 ? page_to_raw_slot[physical_block] : -2;
    if (shared_raw_slot < -1 || shared_raw_slot >= persistent_raw_slots) {
      shared_invalid_state = 1;
    }
    if (shared_invalid_state != 0) {
      atomicExch(staging_overflow, 1);
      atomicExch(raw_pool_overflow, 1);
    }
  }
  ...
  for (int row = threadIdx.x; row < kBlockSize; row += blockDim.x) {
    token_for_row[row] = -1;
  }
  __syncthreads();
  for (int64_t token_idx = threadIdx.x; token_idx < num_tokens;
       token_idx += blockDim.x) {
    const int64_t slot_idx = slot_mapping[token_idx];
    if (slot_idx >= 0 && slot_idx / kBlockSize == physical_block) {
      token_for_row[slot_idx % kBlockSize] = static_cast<int32_t>(token_idx);
    }
  }
  __syncthreads();
  ...
  uint8_t* __restrict__ staging =
      raw_staging +
      static_cast<int64_t>(staging_slot) * raw_staging_stride_slot;
  const uint8_t* __restrict__ compact_page =
      kv_cache + static_cast<int64_t>(physical_block) * kv_cache_stride_block;
  const uint8_t* __restrict__ raw_page =
      shared_raw_slot >= 0
          ...
          : nullptr;
  const int64_t work_items =
      static_cast<int64_t>(2) * kNumKvHeads * rows * kHeadDim;
  for (int64_t work_idx =
    ...
    const int64_t staging_offset =
        kv_side == 0
            ...
            : ByteV2DefaultRawStagingLayout::value_offset(head_idx, row, dim);
    uint16_t bits;
    const int32_t source_token = token_for_row[row];
    if (source_token >= 0) {
      const uint16_t* __restrict__ source = kv_side == 0 ? key : value;
      const int64_t stride_token =
          kv_side == 0 ? key_stride_token : value_stride_token;
      ...
      const int64_t stride_dim =
          kv_side == 0 ? key_stride_dim : value_stride_dim;
      bits = source[static_cast<int64_t>(source_token) * stride_token +
                    head_idx * stride_head + dim * stride_dim];
    } else if (raw_page != nullptr) {
      bits = byte_v2_load_u16_bytes(raw_page, staging_offset);
    } else {
      const float decoded =
          kv_side == 0
              ? byte_v2_load_payload_elem<ByteV2DefaultLayout, false>(
    ...
    byte_v2_store_u16_bytes(staging, staging_offset, bits);
  }
  __syncthreads();

  if (threadIdx.x == 0) {
    const int32_t prior = atomicSub(valid_rows + staging_slot, kCompletionUnit);
    const int32_t remaining = prior >> kCompletionShift;
    if ((prior & kRowsMask) != rows || remaining <= 0 ||
        remaining > static_cast<int32_t>(gridDim.x)) {
      atomicExch(staging_overflow, 1);
      atomicExch(raw_pool_overflow, 1);
      __trap();
    }
    shared_is_last = remaining == 1;
  }
  __syncthreads();
  if (shared_is_last != 0) {
    for (int metadata_byte = threadIdx.x;
    ...
    if (threadIdx.x == 0) {
      const int32_t prior =
          atomicCAS(block_to_staging_slot + physical_block, -2, staging_slot);
      if (prior != -2) {
        atomicExch(staging_overflow, 1);
        atomicExch(raw_pool_overflow, 1);
        __trap();
...
template <bool AlignedRawStaging>
__global__ void byte_v2_hydrate_append_multi_token_hybrid_staging_kernel(
    const uint16_t* __restrict__ key, const uint16_t* __restrict__ value,
    uint8_t* __restrict__ raw_staging, uint8_t* __restrict__ kv_cache,
    const uint8_t* __restrict__ persistent_raw_staging,
    const int64_t* __restrict__ slot_mapping,
  ...
  constexpr int kHeadDimV = ByteV2DefaultPolicy::HeadDimV;
  constexpr int kNumKvHeads = ByteV2DefaultLayout::NumKvHeadsValue;
  constexpr int kTokensPerOwnerChunk = kBlockSize;
  constexpr int32_t kClaimedStagingSlot = -2;

  __shared__ int32_t candidate_physical_blocks[kTokensPerOwnerChunk];
  __shared__ int32_t token_for_row[kBlockSize];
  __shared__ int32_t candidate_count;
  __shared__ int32_t is_page_owner;
  __shared__ int32_t owned_staging_slot;
  __shared__ int32_t owned_raw_slot;
  __shared__ int32_t owned_valid_rows;
  __shared__ int32_t invalid_state;
  __shared__ int32_t full_aligned_chunk;

  const int64_t chunk_start =
      static_cast<int64_t>(blockIdx.x) * kTokensPerOwnerChunk;
  const int64_t chunk_end = min(chunk_start + kTokensPerOwnerChunk, num_tokens);
  ...
  if (threadIdx.x == 0) {
    candidate_count = 0;
    invalid_state = 0;
    full_aligned_chunk = chunk_end - chunk_start == kBlockSize ? 1 : 0;
    if (atomicAdd(raw_pool_overflow, 0) != 0 ||
        atomicAdd(staging_overflow, 0) != 0) {
      invalid_state = 1;
    }
    for (int64_t token_idx = chunk_start; token_idx < chunk_end; ++token_idx) {
      const int64_t slot_idx = slot_mapping[token_idx];
      if (slot_idx < 0) {
        full_aligned_chunk = 0;
        continue;
      }
      const int64_t physical_block = slot_idx / kBlockSize;
      if (physical_block < 0 || physical_block >= num_physical_blocks) {
      ...
      const int32_t physical_block32 = static_cast<int32_t>(physical_block);
      if (slot_idx % kBlockSize != token_idx - chunk_start) {
        full_aligned_chunk = 0;
      }
      ...
      for (int candidate_idx = 0; candidate_idx < candidate_count;
           ++candidate_idx) {
        already_listed |=
            candidate_physical_blocks[candidate_idx] == physical_block32;
      }
      if (!already_listed) {
        candidate_physical_blocks[candidate_count++] = physical_block32;
      }
    }
  ...
  // Ensure every thread has completed the initial status read before thread 0
  // reuses the shared flag for the first candidate page.
  __syncthreads();

  for (int candidate_idx = 0; candidate_idx < candidate_count;
       ++candidate_idx) {
    const int32_t physical_block = candidate_physical_blocks[candidate_idx];
    if (threadIdx.x == 0) {
      invalid_state = 0;
      owned_staging_slot = -1;
      owned_raw_slot = -1;
      owned_valid_rows = 0;
      const int32_t prior = atomicCAS(block_to_staging_slot + physical_block,
                                      -1, kClaimedStagingSlot);
      is_page_owner = prior == -1;
      if (prior < kClaimedStagingSlot || prior >= num_staging_slots) {
        invalid_state = 1;
      }
      if (is_page_owner != 0) {
        const int32_t staging_slot = atomicAdd(next_staging_slot, 1);
        if (staging_slot < 0 || staging_slot >= num_staging_slots) {
          invalid_state = 1;
        } else {
          owned_staging_slot = staging_slot;
          staging_to_physical_block[staging_slot] = physical_block;
          valid_rows[staging_slot] = 0;
        }
        const int32_t raw_slot = page_to_raw_slot[physical_block];
    ...
    for (int64_t token_idx = threadIdx.x; token_idx < num_tokens;
      ...
      const int row = static_cast<int>(slot_idx % kBlockSize);
      const int32_t prior =
          atomicCAS(token_for_row + row, -1, static_cast<int32_t>(token_idx));
      if (prior != -1) {
        atomicExch(&invalid_state, 1);
        atomicExch(staging_overflow, 1);
        atomicExch(raw_pool_overflow, 1);
      }
      atomicMax(&owned_valid_rows, row + 1);
    }
    __syncthreads();
    if (invalid_state != 0 || owned_valid_rows <= 0 ||
        owned_valid_rows > kBlockSize) {
      if (threadIdx.x == 0) {
        atomicExch(staging_overflow, 1);
        atomicExch(raw_pool_overflow, 1);
      }
    ...
    uint8_t* __restrict__ staging =
        raw_staging +
        static_cast<int64_t>(owned_staging_slot) * raw_staging_stride_slot;
    const uint8_t* __restrict__ compact_page =
        kv_cache + static_cast<int64_t>(physical_block) * kv_cache_stride_block;
    const uint8_t* __restrict__ raw_page =
        owned_raw_slot >= 0
            ? persistent_raw_staging + static_cast<int64_t>(owned_raw_slot) *
                                           persistent_raw_stride_slot
            : nullptr;
    const int64_t work_items =
        static_cast<int64_t>(2) * kNumKvHeads * owned_valid_rows * kHeadDim;
    bool used_full_page_vector_copy = false;
    if constexpr (AlignedRawStaging) {
      if (full_aligned_chunk != 0 && owned_valid_rows == kBlockSize) {
        constexpr int kElemsPerVector = sizeof(uint4) / sizeof(uint16_t);
        static_assert(kHeadDim % kElemsPerVector == 0);
        static_assert(kHeadDimV == kHeadDim);
        constexpr int kDimVectors = kHeadDim / kElemsPerVector;
        constexpr int64_t kVectorWorkItems =
            static_cast<int64_t>(2) * kNumKvHeads * kBlockSize * kDimVectors;
        for (int64_t vector_idx = threadIdx.x; vector_idx < kVectorWorkItems;
             vector_idx += blockDim.x) {
          int64_t tmp = vector_idx;
          const int dim = static_cast<int>(tmp % kDimVectors) * kElemsPerVector;
          tmp /= kDimVectors;
          const int row = static_cast<int>(tmp % kBlockSize);
          tmp /= kBlockSize;
          const int head_idx = static_cast<int>(tmp % kNumKvHeads);
          tmp /= kNumKvHeads;
          const int kv_side = static_cast<int>(tmp);
          ...
          const int64_t stride_dim =
              kv_side == 0 ? key_stride_dim : value_stride_dim;
          const int64_t source_offset = (chunk_start + row) * stride_token +
          ...
          const int64_t staging_offset =
              kv_side == 0 ? ByteV2DefaultRawStagingLayout::key_offset(head_idx,
                           ...
                           : ByteV2DefaultRawStagingLayout::value_offset(
                                 head_idx, row, dim);
          *reinterpret_cast<uint4*>(staging + staging_offset) =
              *reinterpret_cast<const uint4*>(source + source_offset);
        }
        used_full_page_vector_copy = true;
      }
    }
    if (!used_full_page_vector_copy) {
      for (int64_t work_idx = threadIdx.x; work_idx < work_items;
           work_idx += blockDim.x) {
        int64_t tmp = work_idx;
        const int dim = tmp % kHeadDim;
        tmp /= kHeadDim;
        const int row = tmp % owned_valid_rows;
        tmp /= owned_valid_rows;
        const int head_idx = tmp % kNumKvHeads;
        tmp /= kNumKvHeads;
        const int kv_side = tmp & 1;
        if (kv_side != 0 && dim >= kHeadDimV) {
        ...
        if (source_token >= 0) {
          ...
          bits = byte_v2_load_raw_staging_u16<AlignedRawStaging>(
              raw_page, staging_offset);
        } else {
          const float decoded =
              kv_side == 0
                  ? byte_v2_load_payload_elem<ByteV2DefaultLayout, false>(
        ...
        byte_v2_store_raw_staging_u16<AlignedRawStaging>(staging,
                                                         staging_offset, bits);
      }
    }
    __syncthreads();
    ...
    __syncthreads();
    if (threadIdx.x == 0) {
      valid_rows[owned_staging_slot] = owned_valid_rows;
      atomicExch(block_to_staging_slot + physical_block, owned_staging_slot);
    }
    __syncthreads();
  }
}
...
__global__ void byte_v2_commit_raw_staging_to_cache_kernel(
    ...
    int64_t raw_tail_persistent_slots, int static_k_high7_base = -1,
  ...
  static_assert(!(StaticW8 && WarpParallelHistogram));
  ...
  __shared__ int shared_base;
  __shared__ int shared_fallback;
  __shared__ int shared_has_overlay;
  __shared__ int shared_outlier_count;
  __shared__ int shared_warp_outlier_counts[4];
  extern __shared__ int shared_high_counts[];

  int parallel_best_base = 0;
  int parallel_outlier_count = 0;
  int parallel_fallback = 0;
  if constexpr (StaticW8) {
    const int best_base =
        kv_side == 0 ? static_k_high7_base : static_v_high7_base;
    const uint8_t* __restrict__ staging =
        raw_staging + staging_slot * raw_staging_stride_slot;
    const int elem_count = rows * kCodecDimBlock;
    int thread_outliers = 0;
    for (int elem_idx = threadIdx.x; elem_idx < elem_count;
         elem_idx += blockDim.x) {
      ...
      const int64_t offset =
          kv_side == 0
              ...
              : ByteV2DefaultRawStagingLayout::value_offset(head_idx, row, dim);
      const uint16_t bits =
          byte_v2_load_raw_staging_u16<AlignedRawStaging>(staging, offset);
      thread_outliers += !byte_v2_high7_in_window(bits, best_base);
    }
    const int lane = threadIdx.x % 32;
    const int warp = threadIdx.x / 32;
#pragma unroll
    for (int offset = 16; offset > 0; offset /= 2) {
      thread_outliers += __shfl_down_sync(0xffffffffu, thread_outliers, offset);
    }
    if (lane == 0) {
      shared_warp_outlier_counts[warp] = thread_outliers;
    }
    __syncthreads();
    if (threadIdx.x == 0) {
      parallel_best_base = best_base;
      for (int warp_idx = 0; warp_idx < blockDim.x / 32; ++warp_idx) {
        parallel_outlier_count += shared_warp_outlier_counts[warp_idx];
      }
      parallel_fallback = parallel_outlier_count >
                          ByteV2DefaultLayout::OutlierEntriesPerTileValue;
    }
  } else if constexpr (WarpParallelHistogram) {
    shared_high_counts[threadIdx.x] = 0;
    __syncthreads();
...
#pragma unroll
  ...
  if (threadIdx.x == 0) {
    ...
    const uint8_t* __restrict__ staging =
        raw_staging + staging_slot * raw_staging_stride_slot;
    if constexpr (StaticW8 || WarpParallelHistogram) {
    ...
    if (has_overlay) {
      outlier_pool_index =
          byte_v2_try_allocate_outlier_segment<ByteV2DefaultLayout>(
              page, outlier_count);
      if (outlier_pool_index < 0) {
        // The compact page is no longer authoritative. Leave an explicit
        // fatal marker and let the hybrid commit publish the exact raw page.
        fallback = 1;
        has_overlay = 0;
      }
    }
    ...
    shared_base = best_base;
    shared_fallback = fallback;
    shared_has_overlay = has_overlay;
    shared_outlier_count = outlier_count;
  }
  __syncthreads();
      ...
      if (active_pair) {
        ...
        const int64_t offset1 =
            kv_side == 0
                ...
                : ByteV2DefaultRawStagingLayout::value_offset(head_idx, row,
                                                              dim1);
        bits0 = byte_v2_load_aligned_u16(staging, offset0);
        bits1 = byte_v2_load_aligned_u16(staging, offset1);
      }
      const int outlier0 =
          active_pair && !byte_v2_high7_in_window(bits0, shared_base);
      const int outlier1 =
          active_pair && !byte_v2_high7_in_window(bits1, shared_base);
      const int thread_outliers = outlier0 + outlier1;
      const int lane = threadIdx.x % 32;
      const int warp = threadIdx.x / 32;
      int inclusive_outliers = thread_outliers;
#pragma unroll
      for (int offset = 1; offset < 32; offset *= 2) {
        const int prior =
            __shfl_up_sync(0xffffffffu, inclusive_outliers, offset);
        if (lane >= offset) {
          inclusive_outliers += prior;
        }
      }
      if (lane == 31) {
        shared_warp_outlier_counts[warp] = inclusive_outliers;
      }
      __syncthreads();
      int warp_base = 0;
      for (int prior_warp = 0; prior_warp < warp; ++prior_warp) {
        warp_base += shared_warp_outlier_counts[prior_warp];
      }
      const int thread_base = warp_base + inclusive_outliers - thread_outliers;
      if (threadIdx.x == blockDim.x - 1 &&
          thread_base + thread_outliers != shared_outlier_count) {
        __trap();
      }
      const int token_tile = 0;
      if (outlier0 != 0) {
        const int elem_idx = row * kCodecDimBlock + pair_in_dim_tile * 2;
        const uint16_t entry = static_cast<uint16_t>(
            ByteV2DefaultLayout::OutlierEntryPolicy::encode(
                elem_idx, static_cast<int>(bits0 >> 8)));
        const int entry_idx = thread_base;
        const int entry_offset =
            kv_side == 0 ? ByteV2DefaultLayout::k_outlier_payload_offset(
                               page, head_idx, dim_tile, token_tile, entry_idx)
                         : ByteV2DefaultLayout::v_outlier_payload_offset(
                               page, head_idx, dim_tile, token_tile, entry_idx);
        byte_v2_store_u16_bytes(page, entry_offset, entry);
      }
      if (outlier1 != 0) {
        const int elem_idx = row * kCodecDimBlock + pair_in_dim_tile * 2 + 1;
        const uint16_t entry = static_cast<uint16_t>(
            ByteV2DefaultLayout::OutlierEntryPolicy::encode(
                elem_idx, static_cast<int>(bits1 >> 8)));
        const int entry_idx = thread_base + outlier0;
        const int entry_offset =
            kv_side == 0 ? ByteV2DefaultLayout::k_outlier_payload_offset(
                               page, head_idx, dim_tile, token_tile, entry_idx)
                         : ByteV2DefaultLayout::v_outlier_payload_offset(
  ...
  if (active_pair) {
    ...
    const uint16_t bits0 =
        byte_v2_load_raw_staging_u16<AlignedRawStaging>(staging, offset0);
    const uint16_t bits1 =
        byte_v2_load_raw_staging_u16<AlignedRawStaging>(staging, offset1);
...
__global__ void byte_v2_persist_raw_fallback_pages_kernel(
  ...
  __shared__ int32_t shared_raw_slot;
  __shared__ int32_t shared_should_persist;
  if (threadIdx.x == 0) {
    shared_raw_slot = -1;
    shared_should_persist = 0;
    const int32_t physical_block = staging_to_physical_block[staging_slot];
    ...
    if (physical_block >= 0 && physical_block < num_physical_blocks &&
      ...
      if (raw_slot < -1) {
        atomicExch(raw_pool_overflow, 1);
        shared_should_persist = 1;
      } else if (raw_slot >= 0 || compact_pool_overflow != 0 ||
                 page_stats.fallback_tiles != 0) {
        if (raw_slot < 0) {
          int32_t observed = atomicAdd(free_raw_slot_count, 0);
          while (observed > 0) {
            if (observed > persistent_raw_slots) {
...
__device__ __forceinline__ void byte_v2_reset_raw_fallback_page(
    int32_t* __restrict__ page_to_raw_slot,
    int32_t* __restrict__ free_raw_slots,
    int32_t* __restrict__ free_raw_slot_count,
    int32_t* __restrict__ raw_pool_overflow, int32_t physical_block,
    int64_t num_physical_blocks, int64_t persistent_raw_slots) {
  if (physical_block < 0 || physical_block >= num_physical_blocks) {
    atomicExch(raw_pool_overflow, 1);
    __trap();
    return;
  ...
  const int32_t raw_slot = atomicExch(page_to_raw_slot + physical_block, -1);
  if (raw_slot == -1) {
    return;
  }
  if (raw_slot < 0 || raw_slot >= persistent_raw_slots) {
  ...
  // Reset runs as a separate kernel on the same stream as updates and
  // attention. The kernel boundary orders these slot writes before a later
  // allocator pop. atomicExch above makes duplicate block IDs idempotent.
  const int32_t free_index = atomicAdd(free_raw_slot_count, 1);
  if (free_index < 0 || free_index >= persistent_raw_slots) {
    atomicExch(raw_pool_overflow, 1);
    __trap();
...
__global__ void byte_v2_reset_raw_fallback_pages_kernel(
    ...
    int32_t* __restrict__ free_raw_slot_count,
    int32_t* __restrict__ raw_pool_overflow,
    const int32_t* __restrict__ physical_block_ids, int64_t num_block_ids,
    int64_t num_physical_blocks, int64_t persistent_raw_slots) {
  for (int64_t index = blockIdx.x * blockDim.x + threadIdx.x;
       index < num_block_ids;
       index += static_cast<int64_t>(blockDim.x) * gridDim.x) {
    const int32_t physical_block = physical_block_ids[index];
    byte_v2_reset_raw_fallback_page(page_to_raw_slot, free_raw_slots,
                                    free_raw_slot_count, raw_pool_overflow,
                                    physical_block, num_physical_blocks,
                                    persistent_raw_slots);
  }
}

constexpr int kByteV2MaxBatchedRawFallbackLayers = 64;

struct ByteV2RawFallbackResetLayer {
  int32_t* page_to_raw_slot;
  int32_t* free_raw_slots;
  int32_t* free_raw_slot_count;
  int32_t* raw_pool_overflow;
  int64_t num_physical_blocks;
  int64_t persistent_raw_slots;
};

struct ByteV2RawFallbackResetBatch {
  ByteV2RawFallbackResetLayer layers[kByteV2MaxBatchedRawFallbackLayers];
};

__global__ void byte_v2_reset_raw_fallback_pages_batched_kernel(
    ByteV2RawFallbackResetBatch batch, int32_t num_layers,
    const int32_t* __restrict__ physical_block_ids, int64_t num_block_ids) {
  const int64_t total_work = num_block_ids * static_cast<int64_t>(num_layers);
  for (int64_t linear_index = blockIdx.x * blockDim.x + threadIdx.x;
       linear_index < total_work;
       linear_index += static_cast<int64_t>(blockDim.x) * gridDim.x) {
    // Layers are the contiguous dimension. For the common 32-layer model,
    // one warp reads a single physical ID and CUDA broadcasts that load.
    const int64_t block_id_index = linear_index / num_layers;
    const int32_t layer_index = linear_index % num_layers;
    const int32_t physical_block = physical_block_ids[block_id_index];
    const ByteV2RawFallbackResetLayer& layer = batch.layers[layer_index];
    byte_v2_reset_raw_fallback_page(
        layer.page_to_raw_slot, layer.free_raw_slots, layer.free_raw_slot_count,
        layer.raw_pool_overflow, physical_block, layer.num_physical_blocks,
        layer.persistent_raw_slots);
  }
}
...
void byte_v2_prepare_raw_staging(
  ...
  const int64_t num_page_groups =
      (num_tokens + ByteV2DefaultPolicy::AllocBlockTokens - 1) /
      ByteV2DefaultPolicy::AllocBlockTokens;
  const bool use_page_parallel =
      byte_v2_page_parallel_raw_staging_prepare_enabled() &&
      num_tokens >= kByteV2PageParallelPrepareMinTokens &&
      num_page_groups <= staging_to_physical_block.size(0) &&
      num_page_groups <= std::numeric_limits<int32_t>::max() - 1;
  if (use_page_parallel) {
    const int64_t validate_blocks64 =
        (num_page_groups + kWarpsPerBlock - 1) / kWarpsPerBlock;
    const int validate_blocks =
        static_cast<int>(validate_blocks64 > 65535 ? 65535 : validate_blocks64);
    byte_v2_validate_page_parallel_raw_staging_kernel<<<validate_blocks,
                                                        kThreads, 0, stream>>>(
        slot_mapping.const_data_ptr<int64_t>(),
        ...
        block_to_staging_slot.size(0), num_page_groups);
    const int64_t finalize_blocks64 =
        (num_page_groups + kThreads - 1) / kThreads;
    const int finalize_blocks =
        static_cast<int>(finalize_blocks64 > 65535 ? 65535 : finalize_blocks64);
    byte_v2_finalize_page_parallel_raw_staging_kernel<<<finalize_blocks,
                                                        kThreads, 0, stream>>>(
        block_to_staging_slot.mutable_data_ptr<int32_t>(),
        staging_to_physical_block.mutable_data_ptr<int32_t>(),
        valid_rows.mutable_data_ptr<int32_t>(),
        next_staging_slot.mutable_data_ptr<int32_t>(),
        overflow.mutable_data_ptr<int32_t>(), num_page_groups,
        block_to_staging_slot.size(0));
    byte_v2_prepare_raw_staging_serial_fallback_kernel<<<1, 1, 0, stream>>>(
        slot_mapping.const_data_ptr<int64_t>(),
        block_to_staging_slot.mutable_data_ptr<int32_t>(),
        staging_to_physical_block.mutable_data_ptr<int32_t>(),
        valid_rows.mutable_data_ptr<int32_t>(),
    ...
    byte_v2_prepare_raw_staging_kernel<<<1, 1, 0, stream>>>(
        slot_mapping.const_data_ptr<int64_t>(),
        block_to_staging_slot.mutable_data_ptr<int32_t>(),
        staging_to_physical_block.mutable_data_ptr<int32_t>(),
        valid_rows.mutable_data_ptr<int32_t>(),
...
static void byte_v2_hydrate_raw_staging_from_cache_impl(
  ...
  STD_TORCH_CHECK(
      (persistent_raw_staging == nullptr) == (page_to_raw_slot == nullptr),
      "persistent raw staging and page map must be provided "
      "together");
  ...
  if (persistent_raw_staging != nullptr) {
    STD_TORCH_CHECK(
        persistent_raw_staging->device() == raw_staging.device() &&
            page_to_raw_slot->device() == raw_staging.device(),
        "persistent raw staging, page map, and transient staging must be on "
        "the same device");
  }
  check_byte_v2_tile_policy({codec_token_block, codec_dim_block,
                             alloc_block_tokens, alloc_block_tokens,
  ...
  STD_TORCH_CHECK(valid_rows.scalar_type() == ScalarType::Int,
                  "valid_rows must be int32");
  if (persistent_raw_staging != nullptr) {
    STD_TORCH_CHECK(persistent_raw_staging->scalar_type() == ScalarType::Byte,
                    "persistent_raw_staging must use uint8 storage");
    STD_TORCH_CHECK(page_to_raw_slot->scalar_type() == ScalarType::Int,
                    "page_to_raw_slot must be int32");
  }
  STD_TORCH_CHECK(raw_staging.dim() == 2,
                  "raw_staging must be [num_slots, slot_size_bytes]");
  STD_TORCH_CHECK(kv_cache.dim() == 2,
  ...
  if (persistent_raw_staging != nullptr) {
    STD_TORCH_CHECK(
        persistent_raw_staging->dim() == 2,
        "persistent_raw_staging must be [num_raw_slots, slot_size_bytes]");
    STD_TORCH_CHECK(page_to_raw_slot->dim() == 1 &&
                        page_to_raw_slot->size(0) == kv_cache.size(0),
                    "page_to_raw_slot must contain one entry per KV block");
  }
  STD_TORCH_CHECK(valid_rows.size(0) >= staging_to_physical_block.size(0),
                  "valid_rows must contain all staging slots");
  ...
  STD_TORCH_CHECK(kv_cache.stride(1) == 1,
                  "kv_cache page dimension must be contiguous");
  if (persistent_raw_staging != nullptr) {
    STD_TORCH_CHECK(persistent_raw_staging->size(0) > 0,
                    "persistent_raw_staging must contain at least one slot");
    STD_TORCH_CHECK(
        persistent_raw_staging->size(1) >=
            ByteV2DefaultRawStagingLayout::SlotSizeBytes,
        "persistent raw slot is smaller than the ByteV2 raw layout");
    STD_TORCH_CHECK(persistent_raw_staging->stride(0) >=
                            ByteV2DefaultRawStagingLayout::SlotSizeBytes &&
                        persistent_raw_staging->stride(1) == 1,
                    "persistent raw slots must be contiguous");
    STD_TORCH_CHECK(page_to_raw_slot->stride(0) == 1,
                    "page_to_raw_slot must be contiguous");
  }
  STD_TORCH_CHECK(
      codec_token_block == ByteV2DefaultPolicy::CodecTokenBlock &&
          codec_dim_block == ByteV2DefaultPolicy::CodecDimBlock &&
  ...
  byte_v2_hydrate_raw_staging_from_cache_kernel<<<blocks, kThreads, 0,
      ...
      persistent_raw_staging == nullptr
          ? nullptr
          : reinterpret_cast<const uint8_t*>(
                persistent_raw_staging->const_data_ptr()),
      page_to_raw_slot == nullptr ? nullptr
                                  : page_to_raw_slot->const_data_ptr<int32_t>(),
      persistent_raw_staging == nullptr ? 0 : persistent_raw_staging->size(0),
      persistent_raw_staging == nullptr ? 0
...
void byte_v2_hydrate_raw_staging_from_cache(
    ...
    torch::stable::Tensor& valid_rows, int64_t codec_token_block,
    int64_t codec_dim_block, int64_t alloc_block_tokens) {
  byte_v2_hydrate_raw_staging_from_cache_impl(
      raw_staging, kv_cache, staging_to_physical_block, valid_rows,
      codec_token_block, codec_dim_block, alloc_block_tokens, nullptr, nullptr);
}
...
void byte_v2_hydrate_raw_staging_from_hybrid_cache(
  ...
  byte_v2_hydrate_raw_staging_from_cache_impl(
      ...
      &persistent_raw_staging, &page_to_raw_slot);
}
...
static void byte_v2_commit_raw_staging_to_cache_impl(
    ...
    bool warp_parallel_histogram, int64_t static_k_high7_base = -1,
    int64_t static_v_high7_base = -1,
    torch::stable::Tensor* persistent_raw_staging = nullptr,
    torch::stable::Tensor* page_to_raw_slot = nullptr,
    torch::stable::Tensor* free_raw_slots = nullptr,
    torch::stable::Tensor* free_raw_slot_count = nullptr,
    torch::stable::Tensor* raw_pool_overflow = nullptr) {
  using torch::headeronly::ScalarType;

  const bool hybrid_commit = persistent_raw_staging != nullptr;
  const ByteV2StaticW8Bases static_bases =
      byte_v2_static_w8_bases(static_k_high7_base, static_v_high7_base);
  STD_TORCH_CHECK(
      hybrid_commit == (page_to_raw_slot != nullptr) &&
          hybrid_commit == (free_raw_slots != nullptr) &&
          hybrid_commit == (free_raw_slot_count != nullptr) &&
          hybrid_commit == (raw_pool_overflow != nullptr),
      "persistent raw staging, page map, allocator, and overflow flag must "
  ...
  if (hybrid_commit) {
    STD_TORCH_CHECK(
        persistent_raw_staging->device() == raw_staging.device() &&
            page_to_raw_slot->device() == raw_staging.device() &&
            free_raw_slots->device() == raw_staging.device() &&
            free_raw_slot_count->device() == raw_staging.device() &&
            raw_pool_overflow->device() == raw_staging.device(),
        "persistent raw state and transient staging must be on the same "
        "device");
  }
  check_byte_v2_tile_policy({codec_token_block, codec_dim_block,
                             alloc_block_tokens, alloc_block_tokens,
  ...
  if (hybrid_commit) {
    ...
    STD_TORCH_CHECK(page_to_raw_slot->scalar_type() == ScalarType::Int &&
                    ...
                    "persistent raw map and allocator state must be int32");
  }
  STD_TORCH_CHECK(raw_staging.dim() == 2,
                  "raw_staging must be [num_slots, slot_size_bytes]");
  STD_TORCH_CHECK(kv_cache.dim() == 2,
  ...
  if (hybrid_commit) {
    STD_TORCH_CHECK(
        persistent_raw_staging->dim() == 2 &&
            persistent_raw_staging->size(0) > 0 &&
            persistent_raw_staging->size(1) >=
                ByteV2DefaultRawStagingLayout::SlotSizeBytes,
        "persistent_raw_staging must contain complete ByteV2 raw slots");
    STD_TORCH_CHECK(page_to_raw_slot->dim() == 1 &&
                        page_to_raw_slot->size(0) == kv_cache.size(0),
                    "page_to_raw_slot must contain one entry per KV block");
    STD_TORCH_CHECK(
        free_raw_slots->dim() == 1 &&
            free_raw_slots->size(0) == persistent_raw_staging->size(0),
        "free_raw_slots must contain one entry per persistent raw slot");
    STD_TORCH_CHECK(
        free_raw_slot_count->dim() == 1 && free_raw_slot_count->size(0) == 1 &&
            raw_pool_overflow->dim() == 1 && raw_pool_overflow->size(0) == 1,
        "persistent raw allocator state must be scalar tensors");
  }
  STD_TORCH_CHECK(valid_rows.size(0) >= staging_to_physical_block.size(0),
                  "valid_rows must contain all staging slots");
  STD_TORCH_CHECK(raw_staging.size(0) >= staging_to_physical_block.size(0),
  ...
  if (hybrid_commit) {
    STD_TORCH_CHECK(persistent_raw_staging->stride(0) >=
                    ...
                    "persistent raw slots must be contiguous");
    STD_TORCH_CHECK(
        page_to_raw_slot->stride(0) == 1 && free_raw_slots->stride(0) == 1 &&
            free_raw_slot_count->stride(0) == 1 &&
            raw_pool_overflow->stride(0) == 1,
        "persistent raw map and allocator state must be contiguous");
    STD_TORCH_CHECK(
    ...
    if (fuse_metadata_clear) {
      byte_v2_commit_raw_staging_to_cache_kernel<true, false, false, false,
          ...
          <<<grid, kThreads, 0, stream>>>(
              ...
              0, static_bases.k, static_bases.v);
    } else {
      constexpr int kClearThreads = 256;
      const int64_t clear_work_items =
          num_staging_slots * ByteV2DefaultLayout::AlignedMetadataBytes;
...
static void byte_v2_commit_single_token_raw_staging_and_release_impl(
    ...
    int64_t static_k_high7_base = -1, int64_t static_v_high7_base = -1) {
  const ByteV2StaticW8Bases static_bases =
      byte_v2_static_w8_bases(static_k_high7_base, static_v_high7_base);
  const torch::stable::accelerator::DeviceGuard device_guard(
      raw_staging.get_device_index());
  ...
  const dim3 grid(
      ...
      1);
  if (static_bases.enabled) {
    byte_v2_commit_raw_staging_to_cache_kernel<true, false, false, true, false,
                                               false, true>
        <<<grid, kThreads, 0, stream>>>(
            reinterpret_cast<const uint8_t*>(raw_staging.const_data_ptr()),
            reinterpret_cast<uint8_t*>(kv_cache.mutable_data_ptr()),
            ...
            nullptr, nullptr, nullptr, nullptr, nullptr, 0, static_bases.k,
            static_bases.v);
  } else {
...
void byte_v2_commit_raw_staging_to_hybrid_cache(
  ...
  byte_v2_commit_raw_staging_to_cache_impl(
      raw_staging, kv_cache, staging_to_physical_block, valid_rows,
      codec_token_block, codec_dim_block, alloc_block_tokens, false, false,
      false, -1, -1, &persistent_raw_staging, &page_to_raw_slot,
      &free_raw_slots, &free_raw_slot_count, &raw_pool_overflow);
}
...
void byte_v2_reset_raw_fallback_pages(
  ...
  STD_TORCH_CHECK(page_to_raw_slot.device().is_cuda(),
                  "page_to_raw_slot must be a CUDA tensor");
  STD_TORCH_CHECK(
      free_raw_slots.device() == page_to_raw_slot.device() &&
          free_raw_slot_count.device() == page_to_raw_slot.device() &&
          raw_pool_overflow.device() == page_to_raw_slot.device() &&
          physical_block_ids.device() == page_to_raw_slot.device(),
      "raw fallback reset tensors must be on the same device");
  STD_TORCH_CHECK(page_to_raw_slot.scalar_type() == ScalarType::Int &&
                      free_raw_slots.scalar_type() == ScalarType::Int &&
                      free_raw_slot_count.scalar_type() == ScalarType::Int &&
                      raw_pool_overflow.scalar_type() == ScalarType::Int &&
                      physical_block_ids.scalar_type() == ScalarType::Int,
                  "raw fallback reset tensors must use int32");
  STD_TORCH_CHECK(
      page_to_raw_slot.dim() == 1 && free_raw_slots.dim() == 1 &&
          free_raw_slots.size(0) > 0 && free_raw_slot_count.dim() == 1 &&
          free_raw_slot_count.size(0) == 1 && raw_pool_overflow.dim() == 1 &&
          raw_pool_overflow.size(0) == 1 && physical_block_ids.dim() == 1,
      "raw fallback reset tensors have invalid shapes");
  STD_TORCH_CHECK(
      page_to_raw_slot.stride(0) == 1 && free_raw_slots.stride(0) == 1 &&
          free_raw_slot_count.stride(0) == 1 &&
          raw_pool_overflow.stride(0) == 1 && physical_block_ids.stride(0) == 1,
      "raw fallback reset tensors must be contiguous");

  const int64_t num_block_ids = physical_block_ids.size(0);
  if (num_block_ids == 0) {
    return;
  }
  constexpr int kThreads = 256;
  const int64_t blocks64 = (num_block_ids + kThreads - 1) / kThreads;
  const int blocks = static_cast<int>(blocks64 > 65535 ? 65535 : blocks64);
  const torch::stable::accelerator::DeviceGuard device_guard(
      page_to_raw_slot.get_device_index());
  const cudaStream_t stream =
      get_current_cuda_stream(page_to_raw_slot.get_device_index());
  byte_v2_reset_raw_fallback_pages_kernel<<<blocks, kThreads, 0, stream>>>(
      page_to_raw_slot.mutable_data_ptr<int32_t>(),
      free_raw_slots.mutable_data_ptr<int32_t>(),
      free_raw_slot_count.mutable_data_ptr<int32_t>(),
      raw_pool_overflow.mutable_data_ptr<int32_t>(),
      physical_block_ids.const_data_ptr<int32_t>(), num_block_ids,
      page_to_raw_slot.size(0), free_raw_slots.size(0));
  const cudaError_t err = cudaGetLastError();
  STD_TORCH_CHECK(err == cudaSuccess,
                  "byte_v2 raw fallback reset kernel launch failed: ",
                  cudaGetErrorString(err));
}
...
void byte_v2_reset_raw_fallback_pages_batched(
  ...
  const size_t num_layers = page_to_raw_slots.size();
  STD_TORCH_CHECK(
      free_raw_slots.size() == num_layers &&
          free_raw_slot_counts.size() == num_layers &&
          raw_pool_overflows.size() == num_layers,
      "batched raw fallback reset tensor lists must have equal lengths");
  if (num_layers == 0) {
    return;
  }
  STD_TORCH_CHECK(physical_block_ids.device().is_cuda(),
                  "physical_block_ids must be a CUDA tensor");
  STD_TORCH_CHECK(physical_block_ids.scalar_type() == ScalarType::Int &&
                      physical_block_ids.dim() == 1 &&
                      physical_block_ids.stride(0) == 1,
                  "physical_block_ids must be a contiguous int32 vector");

  const auto device = physical_block_ids.device();
  for (size_t layer_index = 0; layer_index < num_layers; ++layer_index) {
    auto& page_to_raw_slot = page_to_raw_slots[layer_index];
    auto& layer_free_raw_slots = free_raw_slots[layer_index];
    auto& free_raw_slot_count = free_raw_slot_counts[layer_index];
    auto& raw_pool_overflow = raw_pool_overflows[layer_index];
    STD_TORCH_CHECK(
        page_to_raw_slot.device() == device &&
            layer_free_raw_slots.device() == device &&
            free_raw_slot_count.device() == device &&
            raw_pool_overflow.device() == device,
        "batched raw fallback reset tensors must be on the same device");
    STD_TORCH_CHECK(page_to_raw_slot.scalar_type() == ScalarType::Int &&
                        layer_free_raw_slots.scalar_type() == ScalarType::Int &&
                        free_raw_slot_count.scalar_type() == ScalarType::Int &&
                        raw_pool_overflow.scalar_type() == ScalarType::Int,
                    "batched raw fallback reset tensors must use int32");
    STD_TORCH_CHECK(
        page_to_raw_slot.dim() == 1 && layer_free_raw_slots.dim() == 1 &&
            layer_free_raw_slots.size(0) > 0 &&
            free_raw_slot_count.dim() == 1 &&
            free_raw_slot_count.size(0) == 1 && raw_pool_overflow.dim() == 1 &&
            raw_pool_overflow.size(0) == 1,
        "batched raw fallback reset tensors have invalid shapes");
    STD_TORCH_CHECK(page_to_raw_slot.stride(0) == 1 &&
                        layer_free_raw_slots.stride(0) == 1 &&
                        free_raw_slot_count.stride(0) == 1 &&
                        raw_pool_overflow.stride(0) == 1,
                    "batched raw fallback reset tensors must be contiguous");
  }
  ...
  constexpr int kThreads = 256;
  const torch::stable::accelerator::DeviceGuard device_guard(
      physical_block_ids.get_device_index());
  const cudaStream_t stream =
      get_current_cuda_stream(physical_block_ids.get_device_index());
  for (size_t layer_start = 0; layer_start < num_layers;
       layer_start += kByteV2MaxBatchedRawFallbackLayers) {
    const int32_t layers_in_batch = static_cast<int32_t>(
        std::min(num_layers - layer_start,
                 static_cast<size_t>(kByteV2MaxBatchedRawFallbackLayers)));
    ByteV2RawFallbackResetBatch batch{};
    for (int32_t local_layer = 0; local_layer < layers_in_batch;
         ++local_layer) {
      const size_t layer_index = layer_start + local_layer;
      auto& page_to_raw_slot = page_to_raw_slots[layer_index];
      auto& layer_free_raw_slots = free_raw_slots[layer_index];
      auto& free_raw_slot_count = free_raw_slot_counts[layer_index];
      auto& raw_pool_overflow = raw_pool_overflows[layer_index];
      batch.layers[local_layer] = {
          page_to_raw_slot.mutable_data_ptr<int32_t>(),
          layer_free_raw_slots.mutable_data_ptr<int32_t>(),
          free_raw_slot_count.mutable_data_ptr<int32_t>(),
          raw_pool_overflow.mutable_data_ptr<int32_t>(),
          page_to_raw_slot.size(0),
          layer_free_raw_slots.size(0),
      };
    }
    STD_TORCH_CHECK(
        num_block_ids <= std::numeric_limits<int64_t>::max() / layers_in_batch,
        "batched raw fallback reset work size overflow");
    const int64_t total_work = num_block_ids * layers_in_batch;
    const int64_t blocks64 = (total_work + kThreads - 1) / kThreads;
    const int blocks = static_cast<int>(blocks64 > 65535 ? 65535 : blocks64);
    byte_v2_reset_raw_fallback_pages_batched_kernel<<<blocks, kThreads, 0,
                                                      stream>>>(
        batch, layers_in_batch, physical_block_ids.const_data_ptr<int32_t>(),
        num_block_ids);
    const cudaError_t err = cudaGetLastError();
    STD_TORCH_CHECK(err == cudaSuccess,
                    "byte_v2 batched raw fallback reset kernel launch failed: ",
                    cudaGetErrorString(err));
  }
}
  ...
  if (fuse_single_token_staging && slot_mapping.dim() == 1 &&
    ...
    if (use_fused_commit_release) {
      byte_v2_commit_single_token_raw_staging_and_release_impl(
          ...
          static_k_high7_base, static_v_high7_base);
    } else {
      byte_v2_commit_raw_staging_to_cache_impl(
          raw_staging, kv_cache, staging_to_physical_block, valid_rows,
          codec_token_block, codec_dim_block, alloc_block_tokens,
...
void byte_v2_update_hybrid_cache_raw_staging_q1(
  ...
  const dim3 commit_grid(
      ...
      1);
  if (static_bases.enabled) {
    byte_v2_commit_raw_staging_to_cache_kernel<true, false, false, false, false,
                                               false, true>
        <<<commit_grid, kCommitThreads, 0, stream>>>(
            reinterpret_cast<const uint8_t*>(raw_staging.const_data_ptr()),
            reinterpret_cast<uint8_t*>(kv_cache.mutable_data_ptr()),
            staging_to_physical_block.mutable_data_ptr<int32_t>(),
            valid_rows.mutable_data_ptr<int32_t>(), 1, raw_staging.stride(0),
            kv_cache.stride(0), nullptr, nullptr, nullptr, nullptr, 0, nullptr,
            nullptr, nullptr, nullptr, nullptr, 0, static_bases.k,
            static_bases.v);
  } else {
    byte_v2_commit_raw_staging_to_cache_kernel<true, false, true, false>
...
void byte_v2_update_hybrid_cache_raw_tail_q1(
  ...
  STD_TORCH_CHECK(
      value.device() == device && raw_staging.device() == device &&
          ...
          (!page_unsafe_flags.has_value() ||
           page_unsafe_flags->device() == device),
      "ByteV2 hybrid raw-tail Q1 tensors must be on the same device");
  STD_TORCH_CHECK(key.scalar_type() == ScalarType::BFloat16 &&
                      value.scalar_type() == ScalarType::BFloat16,
                  "ByteV2 hybrid raw-tail Q1 requires bf16 K/V");
  STD_TORCH_CHECK(raw_staging.scalar_type() == ScalarType::Byte &&
                      kv_cache.scalar_type() == ScalarType::Byte &&
                      persistent_raw_staging.scalar_type() == ScalarType::Byte,
                  "ByteV2 hybrid raw-tail Q1 requires uint8 cache storage");
  STD_TORCH_CHECK(slot_mapping.scalar_type() == ScalarType::Long,
                  "slot_mapping must be int64");
  STD_TORCH_CHECK(
      block_to_staging_slot.scalar_type() == ScalarType::Int &&
          ...
          (!page_unsafe_flags.has_value() ||
           page_unsafe_flags->scalar_type() == ScalarType::Int),
      "ByteV2 hybrid raw-tail Q1 maps and allocator state must be int32");

  STD_TORCH_CHECK(key.dim() == 3 && value.dim() == 3 && key.size(0) == 1 &&
                      value.size(0) == 1 &&
                      key.size(1) == ByteV2DefaultLayout::NumKvHeadsValue &&
                      value.size(1) == key.size(1) &&
                      key.size(2) == ByteV2DefaultPolicy::HeadDim &&
                      value.size(2) == ByteV2DefaultPolicy::HeadDimV,
                  "ByteV2 hybrid raw-tail Q1 requires one 8-head D128 token");
  STD_TORCH_CHECK(slot_mapping.dim() == 1 && slot_mapping.size(0) == 1 &&
                      slot_mapping.stride(0) == 1,
                  "slot_mapping must contain one contiguous token");
  STD_TORCH_CHECK(
  ...
  STD_TORCH_CHECK(
      block_to_staging_slot.dim() == 1 &&
          ...
          next_staging_slot.stride(0) == 1 && overflow.dim() == 1 &&
          overflow.size(0) == 1 && overflow.stride(0) == 1,
      "ByteV2 hybrid raw-tail Q1 transient allocator shapes are invalid");
  STD_TORCH_CHECK(
      page_to_raw_slot.dim() == 1 &&
          page_to_raw_slot.size(0) == kv_cache.size(0) &&
          page_to_raw_slot.stride(0) == 1 && free_raw_slots.dim() == 1 &&
          ...
          free_raw_slot_count.stride(0) == 1 && raw_pool_overflow.dim() == 1 &&
          raw_pool_overflow.size(0) == 1 && raw_pool_overflow.stride(0) == 1,
      "ByteV2 hybrid raw-tail Q1 persistent allocator shapes are invalid");
  if (page_unsafe_flags.has_value()) {
    STD_TORCH_CHECK(page_unsafe_flags->dim() == 1 &&
                        page_unsafe_flags->size(0) >= kv_cache.size(0) &&
                        page_unsafe_flags->stride(0) == 1,
  ...
  constexpr int kTailThreads = 256;
  byte_v2_append_persistent_raw_tail_q1_kernel<<<1, kTailThreads, 0, stream>>>(
      reinterpret_cast<const uint16_t*>(key.const_data_ptr()),
      reinterpret_cast<const uint16_t*>(value.const_data_ptr()),
      reinterpret_cast<uint8_t*>(raw_staging.mutable_data_ptr()),
      reinterpret_cast<uint8_t*>(kv_cache.mutable_data_ptr()),
      reinterpret_cast<uint8_t*>(persistent_raw_staging.mutable_data_ptr()),
      slot_mapping.const_data_ptr<int64_t>(),
      block_to_staging_slot.const_data_ptr<int32_t>(),
      staging_to_physical_block.mutable_data_ptr<int32_t>(),
      valid_rows.mutable_data_ptr<int32_t>(),
      next_staging_slot.mutable_data_ptr<int32_t>(),
      overflow.mutable_data_ptr<int32_t>(),
      ...
      key.stride(1), key.stride(2), value.stride(1), value.stride(2),
      kv_cache.size(0), kv_cache.stride(0), persistent_raw_staging.size(0),
      persistent_raw_staging.stride(0));
  cudaError_t err = cudaGetLastError();
  STD_TORCH_CHECK(err == cudaSuccess,
                  "byte_v2 hybrid raw-tail Q1 append kernel launch failed: ",
                  cudaGetErrorString(err));
    ...
    if (static_bases.enabled) {
      byte_v2_commit_raw_staging_to_cache_kernel<true, false, false, false,
          ...
          <<<commit_grid, kCommitThreads, 0, stream>>>(
              ...
              raw_pool_overflow.mutable_data_ptr<int32_t>(),
              overflow.mutable_data_ptr<int32_t>(),
              persistent_raw_staging.size(0), static_bases.k, static_bases.v);
    } else {
...
static void byte_v2_update_hybrid_cache_raw_staging_multi_token_impl(
  ...
  const ByteV2StaticW8Bases static_bases =
      byte_v2_static_w8_bases(static_k_high7_base, static_v_high7_base);
  STD_TORCH_CHECK(
      !retain_transient_staging || !page_unsafe_flags.has_value(),
      "byte_v2 retained hybrid multi-token update requires deferred page "
      "flag refresh after attention");
  STD_TORCH_CHECK(
      tile_policy[0] == ByteV2DefaultPolicy::CodecTokenBlock &&
          tile_policy[1] == ByteV2DefaultPolicy::CodecDimBlock &&
          tile_policy[2] == ByteV2DefaultPolicy::AllocBlockTokens &&
  ...
  STD_TORCH_CHECK(
      value.device() == device && raw_staging.device() == device &&
          ...
          (!page_unsafe_flags.has_value() ||
           page_unsafe_flags->device() == device),
      "ByteV2 fused hybrid multi-token update tensors must be on the same "
      "device");
  STD_TORCH_CHECK(key.scalar_type() == ScalarType::BFloat16 &&
                      value.scalar_type() == ScalarType::BFloat16,
                  "ByteV2 fused hybrid multi-token update requires bf16 K/V");
  STD_TORCH_CHECK(
      raw_staging.scalar_type() == ScalarType::Byte &&
          kv_cache.scalar_type() == ScalarType::Byte &&
          persistent_raw_staging.scalar_type() == ScalarType::Byte,
      "ByteV2 fused hybrid multi-token update requires uint8 cache storage");
  STD_TORCH_CHECK(slot_mapping.scalar_type() == ScalarType::Long,
                  "slot_mapping must be int64");
  STD_TORCH_CHECK(
      block_to_staging_slot.scalar_type() == ScalarType::Int &&
          ...
          (!page_unsafe_flags.has_value() ||
           page_unsafe_flags->scalar_type() == ScalarType::Int),
      "ByteV2 fused hybrid multi-token maps and allocator state must be "
      "int32");

  STD_TORCH_CHECK(key.dim() == 3 && value.dim() == 3,
                  "key and value must be [tokens, heads, head_dim]");
  STD_TORCH_CHECK(
      key.size(0) > 1 && value.size(0) == key.size(0) &&
          key.size(0) <= std::numeric_limits<int32_t>::max() &&
          key.size(1) == ByteV2DefaultLayout::NumKvHeadsValue &&
          value.size(1) == key.size(1) &&
          key.size(2) == ByteV2DefaultPolicy::HeadDim &&
          value.size(2) == ByteV2DefaultPolicy::HeadDimV,
      "ByteV2 fused hybrid multi-token update requires Q>1 8-head D128 "
      "K/V");
  STD_TORCH_CHECK(
      slot_mapping.dim() == 1 && slot_mapping.size(0) == key.size(0) &&
          slot_mapping.stride(0) == 1,
      "slot_mapping must contain one contiguous entry per input token");
  STD_TORCH_CHECK(
      raw_staging.dim() == 2 && raw_staging.size(0) > 0 &&
          raw_staging.size(0) <= std::numeric_limits<int32_t>::max() &&
          raw_staging.size(1) >= ByteV2DefaultRawStagingLayout::SlotSizeBytes &&
          raw_staging.stride(0) >=
              ByteV2DefaultRawStagingLayout::SlotSizeBytes &&
          raw_staging.stride(1) == 1,
      "raw_staging must contain contiguous ByteV2 raw pages");
  STD_TORCH_CHECK(
      kv_cache.dim() == 2 && kv_cache.size(0) > 0 &&
          kv_cache.size(0) <= std::numeric_limits<int32_t>::max() &&
          kv_cache.size(1) >= ByteV2DefaultLayout::PageSizeBytes &&
  ...
  STD_TORCH_CHECK(
      block_to_staging_slot.dim() == 1 &&
          ...
          block_to_staging_slot.stride(0) == 1 &&
          staging_to_physical_block.dim() == 1 &&
          staging_to_physical_block.size(0) == raw_staging.size(0) &&
          staging_to_physical_block.stride(0) == 1 && valid_rows.dim() == 1 &&
          valid_rows.size(0) == raw_staging.size(0) &&
          valid_rows.stride(0) == 1 && next_staging_slot.dim() == 1 &&
          next_staging_slot.size(0) == 1 && next_staging_slot.stride(0) == 1 &&
          overflow.dim() == 1 && overflow.size(0) == 1 &&
          overflow.stride(0) == 1,
      "ByteV2 fused hybrid multi-token transient allocator shapes are "
      "invalid");
  STD_TORCH_CHECK(
      page_to_raw_slot.dim() == 1 &&
          page_to_raw_slot.size(0) == kv_cache.size(0) &&
          ...
          free_raw_slot_count.stride(0) == 1 && raw_pool_overflow.dim() == 1 &&
          raw_pool_overflow.size(0) == 1 && raw_pool_overflow.stride(0) == 1,
      "ByteV2 fused hybrid multi-token persistent allocator shapes are "
      "invalid");
  if (page_unsafe_flags.has_value()) {
    STD_TORCH_CHECK(page_unsafe_flags->dim() == 1 &&
                        page_unsafe_flags->size(0) >= kv_cache.size(0) &&
  ...
  const bool aligned_raw_staging_access =
      byte_v2_hybrid_full_wave_writer_enabled() && key.stride(2) == 1 &&
      value.stride(2) == 1 && key.stride(0) % 8 == 0 &&
      key.stride(1) % 8 == 0 && value.stride(0) % 8 == 0 &&
      value.stride(1) % 8 == 0 &&
      reinterpret_cast<uintptr_t>(key.const_data_ptr()) % alignof(uint4) == 0 &&
      reinterpret_cast<uintptr_t>(value.const_data_ptr()) % alignof(uint4) == 0;
  const int64_t cooperative_writer_limit =
      byte_v2_cooperative_multi_token_writer_limit();
  const bool within_cooperative_writer_limit =
      key.size(0) <= cooperative_writer_limit &&
      raw_staging.size(0) <= cooperative_writer_limit;
  const bool retained_control_shape =
      key.size(0) <= kByteV2DefaultCooperativeMultiTokenWriterTokens &&
      raw_staging.size(0) <= kByteV2DefaultCooperativeMultiTokenWriterTokens;
  if (within_cooperative_writer_limit &&
      (!retain_transient_staging || retained_control_shape)) {
    ...
    byte_v2_hydrate_append_small_multi_token_hybrid_staging_kernel<<<
        ...
        raw_pool_overflow.mutable_data_ptr<int32_t>(), key.size(0),
        key.stride(0), key.stride(1), key.stride(2), value.stride(0),
        value.stride(1), value.stride(2), kv_cache.size(0), raw_staging.size(0),
        raw_staging.stride(0), kv_cache.stride(0),
        persistent_raw_staging.size(0), persistent_raw_staging.stride(0));
  } else {
    const int stage_blocks = static_cast<int>(
        (key.size(0) + kTokensPerStageBlock - 1) / kTokensPerStageBlock);
    if (aligned_raw_staging_access) {
      byte_v2_hydrate_append_multi_token_hybrid_staging_kernel<true>
          <<<stage_blocks, kStageThreads, 0, stream>>>(
              reinterpret_cast<const uint16_t*>(key.const_data_ptr()),
              reinterpret_cast<const uint16_t*>(value.const_data_ptr()),
              reinterpret_cast<uint8_t*>(raw_staging.mutable_data_ptr()),
              reinterpret_cast<uint8_t*>(kv_cache.mutable_data_ptr()),
              ...
              key.stride(0), key.stride(1), key.stride(2), value.stride(0),
              value.stride(1), value.stride(2), kv_cache.size(0),
              raw_staging.size(0), raw_staging.stride(0), kv_cache.stride(0),
              persistent_raw_staging.size(0), persistent_raw_staging.stride(0));
    } else {
      byte_v2_hydrate_append_multi_token_hybrid_staging_kernel<false>
          <<<stage_blocks, kStageThreads, 0, stream>>>(
              reinterpret_cast<const uint16_t*>(key.const_data_ptr()),
              reinterpret_cast<const uint16_t*>(value.const_data_ptr()),
              reinterpret_cast<uint8_t*>(raw_staging.mutable_data_ptr()),
  ...
  STD_TORCH_CHECK(
      err == cudaSuccess,
      "byte_v2 fused hybrid multi-token staging kernel launch failed: ",
      cudaGetErrorString(err));

  constexpr int kCommitThreads = ByteV2DefaultPolicy::CodecPackedElems;
  constexpr int kHistogramBytes = 128 * sizeof(int);
  const dim3 commit_grid(
      static_cast<unsigned int>(raw_staging.size(0)),
      2 * ByteV2DefaultLayout::NumKvHeadsValue * ByteV2DefaultPolicy::KDimTiles,
      1);
  if (aligned_raw_staging_access) {
    if (static_bases.enabled) {
  ...
  const unsigned int persist_blocks =
      static_cast<unsigned int>(raw_staging.size(0));
  if (retain_transient_staging) {
    // Publish persistent raw fallbacks, but leave the transient page map and
    // allocator state intact for the immediately following hybrid FA2 reader.
    // Its stream-ordered release is responsible for any deferred page flags.
    byte_v2_persist_raw_fallback_pages_kernel<false>
        <<<persist_blocks, kPersistThreads, 0, stream>>>(
            reinterpret_cast<const uint8_t*>(raw_staging.const_data_ptr()),
            reinterpret_cast<const uint8_t*>(kv_cache.const_data_ptr()),
            staging_to_physical_block.mutable_data_ptr<int32_t>(),
            valid_rows.mutable_data_ptr<int32_t>(), raw_staging.size(0),
  ...
  if (retain_transient_staging) {
    STD_TORCH_CHECK(
        err == cudaSuccess,
        "byte_v2 fused hybrid multi-token retained persist kernel launch "
        "failed: ",
        cudaGetErrorString(err));
  } else {
    STD_TORCH_CHECK(
        err == cudaSuccess,
        "byte_v2 fused hybrid multi-token persist/release kernel launch "
        "failed: ",
        cudaGetErrorString(err));
  }
}
...
void byte_v2_update_hybrid_cache_raw_staging_multi_token_retained(
  ...
  byte_v2_update_hybrid_cache_raw_staging_multi_token_impl(
      ...
      /*demote_safe_raw_pages=*/false, static_k_high7_base,
      static_v_high7_base);
}
