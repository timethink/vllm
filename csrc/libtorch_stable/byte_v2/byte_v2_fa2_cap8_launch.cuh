...
//
// Profile-only Static-W16 cap8 FA2 launch path.  It intentionally has a
// distinct kernel and host dispatch so cap8 ptxas compilation cannot perturb
// production ByteV2/SplitZip/shared128 kernels in their original TUs.

#pragma once

#include "byte_v2_fa2_launch.cuh"
#include "byte_v2_fa2_cap8_loader.cuh"

namespace FLASH_NAMESPACE {

DEFINE_FLASH_FORWARD_KERNEL(flash_fwd_splitkv_static_w16_cap8_kernel,
                            bool Is_causal, bool Split,
                            bool ReuseKvSmemNonsplit) {
#if defined(ARCH_SUPPORTS_FLASH)
  using LegacyLoader = std::conditional_t<
      ReuseKvSmemNonsplit,
      vllm::byte_v2::fa2::StaticW16Cap8LoaderReuseKvSmemNonsplit,
      vllm::byte_v2::fa2::StaticW16Cap8Loader>;
  ...
  using ExternalKvLoader = std::conditional_t<
      UseSharedPageDescriptor,
      vllm::byte_v2::fa2::StaticW16Cap8LoaderSharedPageDescriptor,
      LegacyLoader>;
...
void run_flash_static_w16_cap8_splitkv_fwd(Flash_fwd_params& params,
                                           cudaStream_t stream) {
  ...
  static_assert(Kernel_traits::kBlockN ==
                vllm::byte_v2::fa2::StaticW16Cap8Loader::BlockN);
  static_assert(Kernel_traits::kNThreads ==
                vllm::byte_v2::fa2::StaticW16Cap8Loader::Threads);
  ...
  constexpr const char* format_name = "Static-W16-cap8";
  ...
  TORCH_CHECK(params.v_ptr != nullptr && params.v_batch_stride > 0, format_name,
              " requires authoritative raw staging");
  ...
  BOOL_SWITCH(params.num_splits > 1, Split, [&] {
    auto launch = [&](auto reuse_nonsplit_tag) {
      ...
      static_assert(UseSharedPageDescriptor ||
                    ExternalKvLoader::SharedStorageBytes == 0);
      auto kernel =
          &flash_fwd_splitkv_static_w16_cap8_kernel<Kernel_traits, Is_causal,
                                                    Split, ReuseKvSmemNonsplit>;
  ...
  if (params.num_splits > 1) {
    // Preserve the original FA2 split-K combine launch.
...
void run_mha_static_w16_cap8_fwd_splitkv_dispatch(Flash_fwd_params& params,
  ...
  run_flash_static_w16_cap8_splitkv_fwd<KernelTraits, Is_causal>(params,
                                                                 stream);
}
