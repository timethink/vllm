...
"""Profile fused commit/flags/release for the safe ByteV2 n=1 update."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--fused-release", action="store_true")
    parser.add_argument(
        "--comparison",
        action="store_true",
        help=(
            "Benchmark standalone and commit-fused release in one process, "
...
def main() -> None:
    ...
    def update(fused_release: bool) -> None:
        byte_v2_update_cache_raw_staging(
            key,
            value,
            raw_staging,
            ...
            fuse_single_token_commit_release=fused_release,
        )

    if args.comparison and args.profile:
        raise ValueError("--comparison and --profile cannot be used together")
    if args.comparison and args.fused_release:
        raise ValueError("--comparison already benchmarks both release modes")

    if args.comparison:
        for warmup_index in range(args.warmup):
            warmup_order = (False, True) if warmup_index % 2 == 0 else (True, False)
            for fused_release in warmup_order:
                update(fused_release)
    else:
        for _ in range(args.warmup):
            update(args.fused_release)
            ...
            json.dumps(
                {
                    ...
                    "profile": True,
                    "fused_release": args.fused_release,
                },
                sort_keys=True,
            )
        )
    ...
    def measure(fused_release: bool) -> tuple[float, float]:
    ...
    if args.comparison:
        ...
        for trial_index in range(args.trials):
            ...
            for name, fused_release in order:
                event_us, wall_us = measure(fused_release)
    ...
    for _ in range(args.trials):
        event_us, wall_us = measure(args.fused_release)
