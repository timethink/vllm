...
"""Profile stage-fused metadata clear for the safe ByteV2 n=1 update."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--fused-stage-clear", action="store_true")
    parser.add_argument(
        "--comparison",
        action="store_true",
        help=(
            "Benchmark standalone and stage-fused metadata clear in one process, "
...
def main() -> None:
    ...
    def update(fused_stage_clear: bool) -> None:
        byte_v2_update_cache_raw_staging(
            key,
            value,
            raw_staging,
            ...
            fuse_single_token_stage_metadata_clear=fused_stage_clear,
        )

    if args.comparison and args.profile:
        raise ValueError("--comparison and --profile cannot be used together")
    if args.comparison and args.fused_stage_clear:
        raise ValueError("--comparison already benchmarks both clear modes")

    if args.comparison:
        for warmup_index in range(args.warmup):
            warmup_order = (False, True) if warmup_index % 2 == 0 else (True, False)
            for fused_stage_clear in warmup_order:
                update(fused_stage_clear)
    else:
        for _ in range(args.warmup):
            update(args.fused_stage_clear)
            ...
            json.dumps(
                {
                    ...
                    "profile": True,
                    "fused_stage_clear": args.fused_stage_clear,
                },
                sort_keys=True,
            )
        )
    ...
    def measure(fused_stage_clear: bool) -> tuple[float, float]:
    ...
    if args.comparison:
        ...
        for trial_index in range(args.trials):
            ...
            for name, fused_stage_clear in order:
                event_us, wall_us = measure(fused_stage_clear)
    ...
    for _ in range(args.trials):
        event_us, wall_us = measure(args.fused_stage_clear)
