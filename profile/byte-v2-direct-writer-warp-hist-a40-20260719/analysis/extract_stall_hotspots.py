...
def write_report(per_line, out_path, tag, top_n=30):
    ...
    with open(out_path, "w") as f:
        ...
        header = (
            f"{'Rank':>4} {'Total':>10} {'File':<30} "
            f"{'Line':>5}  Breakdown (stall: count)\n"
        )
        f.write(header)
        f.write("-" * 150 + "\n")
        for i, (total, fn, ln, stalls) in enumerate(totals[:top_n]):
            ...
            breakdown = ", ".join(
                ...
                for k, v in sorted(stalls.items(), key=lambda x: -x[1])
                if v
            )
...
def main():
    ...
    ap.add_argument(
        ...
        required=True,
        help=(
            "Path(s) to source-level .ncu-rep file(s). "
            "Pass multiple with repeated flags."
        ),
    )
