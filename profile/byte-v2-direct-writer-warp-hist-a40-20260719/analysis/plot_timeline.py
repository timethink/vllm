...
DEFAULT_METRICS = [
    ...
    # These may or may not populate depending on ncu version, driver, and GPU.
    # The plotter prints "no data" when they are empty.
...
def ascii_plot(vals, label, max_rows=20, max_cols=80):
    ...
    summary = (
        f"  (n={n} active samples, leading_zero={lead}, "
        f"trailing_zero={trail}, max={mx:.3g})"
    )
    lines = [f"\n{label}", summary]
