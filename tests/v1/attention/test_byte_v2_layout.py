...
from vllm.v1.kv_cache_interface import ByteV2FullAttentionSpec, FullAttentionSpec
...
def _compact_outlier_payload_offset(
    cache: torch.Tensor,
...
def _decode_sideband_high_byte_v2_payload(
    ...
    *,
    ...
    if outlier_mask & (1 << tile_index):
        if isinstance(layout, ByteV2PageLayoutV5):
            outlier_payload_offset = _compact_outlier_payload_offset(
                cache,
                layout,
                physical_block=physical_block,
                kv_head=kv_head,
...
def test_page_layout_v6_uses_right_sized_page_outlier_pool():
    layout = ByteV2PageLayoutV6()

    assert layout.metadata_bytes == 896
    assert layout.aligned_metadata_bytes == 896
    assert layout.compressed_payload_bytes == 49152
    assert layout.outlier_pool_entries == 256
    assert layout.outlier_pool_bytes == 512
...
def test_page_layout_v5_remains_the_legacy_splitzip_envelope():
    layout = ByteV2PageLayoutV5()
...
def test_page_layout_v6_derives_tile_descriptors():
    layout = ByteV2PageLayoutV6()
...
def test_byte_v2_backend_is_registered_but_kernel_gated(monkeypatch):
    ...
    assert backend_cls.get_kv_cache_shape(
        ...
        num_kv_heads=8,
        head_size=128,
    ) == (3, ByteV2PageLayoutV6().page_size_bytes)
...
def test_byte_v2_raw_fallback_store_only_allocates_test_diagnostic_when_enabled():
    ...
    kv_cache = torch.empty((2, ByteV2PageLayoutV6().page_size_bytes), dtype=torch.uint8)
...
def test_byte_v2_forward_prefers_fa2_decode_and_slices_metadata(monkeypatch):
    ...
    kv_cache = torch.empty(
        (4, ByteV2PageLayoutV6().page_size_bytes),
        dtype=torch.uint8,
    )
...
def test_byte_v2_static_w16_q1_decode_uses_selected_cascade(monkeypatch):
    impl = object.__new__(byte_v2_attn_module.ByteV2AttentionImpl)
    impl.decode_kernel_mode = "fa2"
    impl.fa2_hybrid_raw_fallback = True
    monkeypatch.setattr(impl, "_fa2_decode_incompatibility", lambda *args: None)
    calls = []

    def cascade(query, kv_cache, output, metadata):
        calls.append((query, kv_cache, output, metadata))
        output.fill_(11)
        return True

    monkeypatch.setattr(impl, "_forward_static_w16_cascade", cascade)
    monkeypatch.setattr(
        impl,
        "_run_hybrid_fa2_attention",
        lambda *args, **kwargs: pytest.fail(
            f"selected Q1 cascade used the regular reader: {args}, {kwargs}"
        ),
    )
    query = torch.empty((16, 32, 128), dtype=torch.bfloat16)
    output = torch.empty_like(query)
    kv_cache = torch.empty((1, 49_792), dtype=torch.uint8)
    metadata = SimpleNamespace(use_cascade=True)

    assert impl._run_fa2_decode(query, kv_cache, output, metadata) is True
    assert len(calls) == 1
    assert calls[0][0] is query
    assert calls[0][1] is kv_cache
...
def test_byte_v2_hybrid_raw_tail_fused_finalize_defaults_on(monkeypatch):
    env_name = "BYTE_V2_HYBRID_RAW_TAIL_FUSED_FINALIZE"
    monkeypatch.delenv(env_name, raising=False)
    assert byte_v2_attn_module._hybrid_raw_tail_fused_finalize_enabled() is True

    monkeypatch.setenv(env_name, "0")
    assert byte_v2_attn_module._hybrid_raw_tail_fused_finalize_enabled() is False


def test_byte_v2_hybrid_raw_mutable_tail_defaults_with_hybrid(monkeypatch):
    env_name = "BYTE_V2_HYBRID_RAW_MUTABLE_TAIL_Q1"
    monkeypatch.delenv(env_name, raising=False)
    assert not byte_v2_attn_module._hybrid_raw_mutable_tail_q1_enabled(
        hybrid_raw_fallback=False
    )
    assert byte_v2_attn_module._hybrid_raw_mutable_tail_q1_enabled(
...
def test_byte_v2_hybrid_raw_mutable_tail_resolves_native_abi(monkeypatch):
    env_name = "BYTE_V2_HYBRID_RAW_MUTABLE_TAIL_Q1"
    monkeypatch.delenv(env_name, raising=False)
    assert not byte_v2_attn_module.resolve_byte_v2_hybrid_raw_mutable_tail_q1(
        native_available=False,
        hybrid_raw_fallback=True,
    )
    assert byte_v2_attn_module.resolve_byte_v2_hybrid_raw_mutable_tail_q1(
    ...
    with pytest.raises(RuntimeError, match="native raw-tail Q1"):
        byte_v2_attn_module.resolve_byte_v2_hybrid_raw_mutable_tail_q1(
            native_available=False,
            hybrid_raw_fallback=True,
        )


def test_byte_v2_hybrid_raw_mutable_tail_requires_hybrid(monkeypatch):
...
def test_byte_v2_forced_raw_promotion_disables_default_raw_tail(
    monkeypatch,
):
    ...
    assert impl.test_forced_raw_promotion is True
    assert impl.hybrid_raw_mutable_tail_q1 is False
...
def test_byte_v2_cached_prefill_hydrate_to_raw_defaults_auto(monkeypatch):
    env_name = "BYTE_V2_FA2_CACHED_PREFILL_HYDRATE_TO_RAW"
    monkeypatch.delenv(env_name, raising=False)
    assert byte_v2_attn_module._cached_prefill_hydrate_to_raw_mode() == "auto"
    assert byte_v2_attn_module._cached_prefill_hydrate_to_raw_enabled() is True

    monkeypatch.setenv(env_name, "1")
    assert byte_v2_attn_module._cached_prefill_hydrate_to_raw_mode() == "enabled"
    assert byte_v2_attn_module._cached_prefill_hydrate_to_raw_enabled() is True

    monkeypatch.setenv(env_name, "0")
    assert byte_v2_attn_module._cached_prefill_hydrate_to_raw_mode() == "disabled"
...
def test_static_w16_initial_prefill_skip_hydrate_defaults_on(monkeypatch):
    env_name = "BYTE_V2_STATIC_W16_INITIAL_PREFILL_SKIP_HYDRATE"
    monkeypatch.delenv(env_name, raising=False)
    assert (
        byte_v2_attn_module._static_w16_initial_prefill_skip_hydrate_enabled() is True
    )

    monkeypatch.setenv(env_name, "0")
    assert (
        byte_v2_attn_module._static_w16_initial_prefill_skip_hydrate_enabled() is False
    )

    monkeypatch.setenv(env_name, "true")
    with pytest.raises(ValueError, match=env_name):
        byte_v2_attn_module._static_w16_initial_prefill_skip_hydrate_enabled()


def test_static_w16_page_aligned_prefill_skip_hydrate_defaults_on(monkeypatch):
    env_name = "BYTE_V2_STATIC_W16_PAGE_ALIGNED_PREFILL_SKIP_HYDRATE"
    monkeypatch.delenv(env_name, raising=False)
    assert (
        byte_v2_attn_module._static_w16_page_aligned_prefill_skip_hydrate_enabled()
        is True
    )
...
def test_static_w16_batched_raw_tail_q1_defaults_on(monkeypatch):
    env_name = "BYTE_V2_STATIC_W16_BATCHED_RAW_TAIL_Q1"
    monkeypatch.delenv(env_name, raising=False)
    assert byte_v2_attn_module._static_w16_batched_raw_tail_q1_enabled() is True

    monkeypatch.setenv(env_name, "0")
    assert byte_v2_attn_module._static_w16_batched_raw_tail_q1_enabled() is False

    monkeypatch.setenv(env_name, "true")
    with pytest.raises(ValueError, match=env_name):
        byte_v2_attn_module._static_w16_batched_raw_tail_q1_enabled()


@pytest.mark.parametrize(
    ("num_tokens", "query_starts", "max_query_len", "expected"),
    [
        (4, (0, 1, 2, 3, 4), 1, True),
        (1, (0, 1), 1, False),
        (4, (0, 1, 1, 3, 4), 1, False),
...
def test_static_w16_batched_q1_requires_one_token_per_request(
    num_tokens,
    query_starts,
    max_query_len,
    expected,
    ...
    assert (
        byte_v2_attn_module._trusted_byte_v2_batched_q1(metadata, num_tokens)
        is expected
    )


def test_static_w16_cascade_attention_defaults_on(monkeypatch):
    env_name = "BYTE_V2_STATIC_W16_CASCADE_ATTENTION"
    monkeypatch.delenv(env_name, raising=False)
    assert byte_v2_attn_module._static_w16_cascade_attention_enabled() is True

    monkeypatch.setenv(env_name, "0")
    assert byte_v2_attn_module._static_w16_cascade_attention_enabled() is False

    monkeypatch.setenv(env_name, "true")
    with pytest.raises(ValueError, match=env_name):
        byte_v2_attn_module._static_w16_cascade_attention_enabled()


def test_static_w16_cascade_prefix_hydrate_defaults_auto(monkeypatch):
    env_name = "BYTE_V2_STATIC_W16_CASCADE_PREFIX_HYDRATE"
    monkeypatch.delenv(env_name, raising=False)
    assert byte_v2_attn_module._static_w16_cascade_prefix_hydrate_mode() == "auto"
    assert byte_v2_attn_module._static_w16_cascade_prefix_hydrate_enabled()

    monkeypatch.setenv(env_name, "0")
    assert byte_v2_attn_module._static_w16_cascade_prefix_hydrate_mode() == "disabled"
    assert not byte_v2_attn_module._static_w16_cascade_prefix_hydrate_enabled()

    monkeypatch.setenv(env_name, "1")
    assert byte_v2_attn_module._static_w16_cascade_prefix_hydrate_mode() == "enabled"
    assert byte_v2_attn_module._static_w16_cascade_prefix_hydrate_enabled()
    ...
    monkeypatch.setenv(env_name, "true")
    with pytest.raises(ValueError, match=env_name):
        byte_v2_attn_module._static_w16_cascade_prefix_hydrate_enabled()


@pytest.mark.parametrize(
    ("mode", "num_tokens", "query_len", "prefix_len", "expected"),
    [
        ("disabled", 32, 1, 8_192, False),
        ("enabled", 1, 16, 16, True),
        ("auto", 16, 1, 1_008, True),
        ("auto", 15, 1, 1_008, False),
        ("auto", 16, 2, 1_008, False),
        ("auto", 16, 1, 992, False),
    ],
)
def test_static_w16_cascade_prefix_hydrate_auto_gate(
    mode,
    num_tokens,
    query_len,
    prefix_len,
    ...
    assert (
        byte_v2_attn_module._static_w16_cascade_prefix_hydrate_for_call(
            mode,
            num_actual_tokens=num_tokens,
...
def test_byte_v2_static_w16_cascade_reads_shared_prefix_once(monkeypatch):
    impl = object.__new__(byte_v2_attn_module.ByteV2AttentionImpl)
    impl.scale = 0.125
    impl.tile_policy = DEFAULT_BYTE_V2_TILE_POLICY
    impl.static_w16_codebook = object()
    state = SimpleNamespace(
        raw_pages=torch.empty((8, 65_536), dtype=torch.uint8),
        page_to_raw_slot=torch.full((24,), -1, dtype=torch.int32),
    )
    impl.raw_fallback_store = SimpleNamespace(state=lambda _: state)

    num_requests = 8
    query = torch.empty((num_requests, 32, 128), dtype=torch.bfloat16)
    output = torch.empty_like(query)
    kv_cache = torch.empty((24, 49_792), dtype=torch.uint8)
    query_start_loc = torch.arange(num_requests + 1, dtype=torch.int32)
    common_blocks = torch.arange(16, dtype=torch.int32)
    block_table = torch.stack(
        [
            torch.cat((common_blocks, torch.tensor([16 + request])))
    ...
    metadata = SimpleNamespace(
        ...
        cu_prefix_query_lens=torch.tensor([0, num_requests], dtype=torch.int32),
        prefix_kv_lens=torch.tensor([256], dtype=torch.int32),
        suffix_kv_lens=torch.ones(num_requests, dtype=torch.int32),
        block_table=block_table,
        seq_lens=torch.full((num_requests,), 257, dtype=torch.int32),
    )
    calls = []
    partials = []

    def attention_with_lse(*args, **kwargs):
        calls.append((args, kwargs))
        partial = torch.full_like(query, len(calls))
        lse = torch.full((32, num_requests), float(len(calls)))
        partials.append((partial, lse))
        return partial, lse
    ...
    monkeypatch.setattr(byte_v2_attn_module, "merge_attn_states", merge)
    monkeypatch.setattr(
        byte_v2_attn_module,
        "byte_v2_static_w16_fa2_paged_attention",
        lambda *args, **kwargs: pytest.fail("cascade used the regular reader"),
    )
    ...
    assert len(calls) == 2
    prefix_args, prefix_kwargs = calls[0]
    assert prefix_args[2] is state.raw_pages
    assert prefix_args[3] is state.page_to_raw_slot
    assert torch.equal(prefix_args[4], metadata.cu_prefix_query_lens)
    assert torch.equal(prefix_args[5], block_table[:1, :16])
    assert torch.equal(prefix_args[6], metadata.prefix_kv_lens)
    ...
    suffix_args, suffix_kwargs = calls[1]
    assert torch.equal(suffix_args[4], query_start_loc)
    assert torch.equal(suffix_args[5], block_table[:, 16:])
    assert torch.equal(suffix_args[6], metadata.suffix_kv_lens)
    assert suffix_kwargs == {
        "scale": 0.125,
        "max_query_len": 1,
        "max_seq_len": 1,
        "causal": True,
...
@pytest.mark.parametrize("requires_release", [True, False])
def test_byte_v2_static_w16_cascade_can_hydrate_shared_prefix(
    monkeypatch,
    requires_release,
):
    impl = object.__new__(byte_v2_attn_module.ByteV2AttentionImpl)
    ...
    impl.tile_policy = DEFAULT_BYTE_V2_TILE_POLICY
    impl.static_w16_codebook = object()
    impl.static_w16_cascade_prefix_hydrate_mode = "enabled"
    impl.static_w16_cascade_prefix_hydrate = True
    state = SimpleNamespace(
        raw_pages=torch.empty((4, 65_536), dtype=torch.uint8),
        page_to_raw_slot=torch.full((8,), -1, dtype=torch.int32),
    )
    impl.raw_fallback_store = SimpleNamespace(state=lambda _: state)

    raw_staging = torch.empty((2, 65_536), dtype=torch.uint8)
    page_map = torch.full((8,), -1, dtype=torch.int32)
    local_block_table = torch.arange(2, dtype=torch.int32).view(1, 2)
    valid_rows = torch.full((2,), 16, dtype=torch.int32)
    calls: list[tuple[Any, ...]] = []

    class FakeManager:
        def stage_cached_prefix(self, **kwargs):
            calls.append(("stage", kwargs))
            return (
                (raw_staging, page_map, local_block_table, valid_rows),
                requires_release,
            )
    ...
    query = torch.empty((num_requests, 32, 128), dtype=torch.bfloat16)
    output = torch.empty_like(query)
    kv_cache = torch.empty((8, 49_792), dtype=torch.uint8)
    query_start_loc = torch.arange(num_requests + 1, dtype=torch.int32)
    block_table = torch.tensor([[3, 1, 4], [3, 1, 5]], dtype=torch.int32)
    metadata = SimpleNamespace(
        use_cascade=True,
        causal=True,
        num_actual_tokens=num_requests,
        ...
        query_start_loc=query_start_loc,
        cu_prefix_query_lens=torch.tensor([0, num_requests], dtype=torch.int32),
        prefix_kv_lens=torch.tensor([32], dtype=torch.int32),
        suffix_kv_lens=torch.ones(num_requests, dtype=torch.int32),
        block_table=block_table,
        seq_lens=torch.full((num_requests,), 33, dtype=torch.int32),
        cascade_prefix_cache_key=17,
    )
    prefix_output = torch.full_like(query, 1)
    prefix_lse = torch.full((32, num_requests), 1.0)
    suffix_output = torch.full_like(query, 2)
    suffix_lse = torch.full((32, num_requests), 2.0)

    def raw_attention(*args, **kwargs):
        calls.append(("raw", args, kwargs))
        return prefix_output, prefix_lse

    def w16_attention(*args, **kwargs):
        calls.append(("w16", args, kwargs))
        return suffix_output, suffix_lse
    ...
    assert impl._forward_static_w16_cascade(query, kv_cache, output, metadata)
    expected_calls = ["stage", "raw"]
    if requires_release:
        expected_calls.append("release")
    expected_calls.extend(("w16", "merge"))
    assert [call[0] for call in calls] == expected_calls
    assert torch.all(output == 7)
    assert torch.equal(calls[0][1]["block_table"], block_table[:1, :2])
    assert calls[0][1]["seq_len"] == 32
    assert calls[0][1]["prefix_cache_key"] == 17
    raw_args, raw_kwargs = calls[1][1:]
    assert raw_args[1] is raw_staging
    assert raw_args[2] is page_map
    assert raw_args[4] is local_block_table
    assert raw_kwargs["block_tables_are_staging_slots"] is True
    assert raw_kwargs["causal"] is False
    if requires_release:
        assert calls[2][1:] == (local_block_table, valid_rows)
    w16_call = next(call for call in calls if call[0] == "w16")
    assert torch.equal(w16_call[1][5], block_table[:, 2:])
...
def test_byte_v2_b1_cached_prefill_defaults_to_hybrid_reader(monkeypatch):
    ...
    result = impl._forward_prefill_from_cache(
        ...
        torch.empty((3, 50_560), dtype=torch.uint8),
        output,
        metadata,
    )
...
def test_byte_v2_b1_cached_prefill_hydrates_then_uses_raw_fa2(monkeypatch):
    ...
    output = torch.empty_like(query)
    kv_cache = torch.empty((5, 50_560), dtype=torch.uint8)
    metadata = SimpleNamespace(
        num_actual_tokens=4,
        query_start_loc=torch.tensor([0, 4], dtype=torch.int32),
        query_start_loc_cpu=torch.tensor([0, 4], dtype=torch.int32),
...
@pytest.mark.parametrize(
    ("query_start_loc_cpu", "is_prefilling"),
    [
        pytest.param(
            torch.tensor([0, 4], dtype=torch.int32),
            None,
            id="missing-prefill-semantics",
        ),
        pytest.param(
            torch.tensor([0, 4], dtype=torch.int32),
            torch.tensor([False], dtype=torch.bool),
            id="async-spec-decode",
        ),
        pytest.param(
            torch.tensor([0, 4], dtype=torch.int32),
            torch.tensor([1], dtype=torch.int32),
            id="wrong-prefill-dtype",
        ),
        pytest.param(
            torch.tensor([0, 4, 4], dtype=torch.int32),
            torch.tensor([True, True], dtype=torch.bool),
            id="prefilling-padded-row",
        ),
    ],
)
def test_byte_v2_b1_cached_prefill_hydrate_requires_explicit_prefill_semantics(
    query_start_loc_cpu,
    is_prefilling,
):
    impl = object.__new__(byte_v2_attn_module.ByteV2AttentionImpl)
    impl.cached_prefill_hydrate_to_raw = True
    impl.cached_prefill_hydrate_to_raw_mode = "enabled"
    impl.raw_fallback_store = object()
    impl.tile_policy = DEFAULT_BYTE_V2_TILE_POLICY
    stage_calls = []
    impl.raw_staging_manager = SimpleNamespace(
        stage_cached_prefill=lambda **kwargs: stage_calls.append(kwargs)
    )
    metadata = SimpleNamespace(
        num_actual_tokens=4,
    ...
    selected = impl._forward_cached_prefill_from_hydrated_cache(
        torch.empty((4, 1, 1), dtype=torch.bfloat16),
        torch.empty((3, 50_560), dtype=torch.uint8),
        torch.empty((4, 1, 1), dtype=torch.bfloat16),
        metadata,
    )
...
def test_byte_v2_b1_cached_prefill_auto_requires_validated_query_size(monkeypatch):
    impl = object.__new__(byte_v2_attn_module.ByteV2AttentionImpl)
    impl.cached_prefill_hydrate_to_raw = True
    impl.cached_prefill_hydrate_to_raw_mode = "auto"
    impl.raw_fallback_store = object()
    impl.tile_policy = DEFAULT_BYTE_V2_TILE_POLICY
    stage_calls = []
    impl.raw_staging_manager = SimpleNamespace(
    ...
    query = torch.empty((4, 1, 1), dtype=torch.bfloat16)
    kv_cache = torch.empty((3, 50_560), dtype=torch.uint8)

    assert not impl._forward_cached_prefill_from_hydrated_cache(
        query,
        kv_cache,
...
def test_byte_v2_hybrid_cached_prefill_rejects_noncausal_metadata():
    ...
    with pytest.raises(RuntimeError, match="require causal attention"):
        impl._forward_prefill_from_cache(
            torch.empty((2, 32, 128), dtype=torch.bfloat16),
            torch.empty((1, 50_560), dtype=torch.uint8),
            torch.empty((2, 32, 128), dtype=torch.bfloat16),
            metadata,
        )
...
def test_byte_v2_hybrid_fa2_wrapper_preserves_mixed_dispatch(monkeypatch):
    ...
    common = (
        output,
        query,
        torch.empty((4, 50_560), dtype=torch.uint8),
...
def test_byte_v2_raw_staging_fa2_with_lse_returns_states(monkeypatch):
    calls = []
    expected_output = torch.empty((2, 32, 128), dtype=torch.bfloat16)
    expected_lse = torch.empty((32, 2), dtype=torch.float32)
    ...
    monkeypatch.setattr(byte_v2_ops_module, "_require_fa2_op", fake_require_fa2_op)
    raw_staging = torch.empty((2, 65_536), dtype=torch.uint8)
    local_block_table = torch.tensor([[0, 1]], dtype=torch.int32)
    result = byte_v2_ops_module.byte_v2_fa2_raw_staging_attention_with_lse(
        torch.empty_like(expected_output),
        raw_staging,
        torch.full((4,), -1, dtype=torch.int32),
    ...
    assert result == (expected_output, expected_lse)
    assert calls[0][3] is None
    assert calls[0][8] is local_block_table
...
def test_byte_v2_hybrid_raw_tail_q1_wrapper_forwards_optional_flags(monkeypatch):
    ...
    byte_v2_ops_module.byte_v2_update_hybrid_cache_raw_tail_q1(
        *tensors,
        tile_policy=(16, 16, 16, 64, 128, 128),
        page_unsafe_flags=flags,
        fuse_commit_finalize=True,
    )

    assert captured["args"][:15] == tuple(tensors)
    assert captured["args"][15] == [16, 16, 16, 64, 128, 128]
    assert captured["args"][16] is flags
    assert captured["args"][17] is True


def test_byte_v2_hybrid_raw_tail_q1_wrapper_omits_disabled_finalize(
    monkeypatch,
):
    captured = {}
    ...
    byte_v2_ops_module.byte_v2_update_hybrid_cache_raw_tail_q1(
        *tensors,
        tile_policy=(16, 16, 16, 64, 128, 128),
        page_unsafe_flags=flags,
        fuse_commit_finalize=False,
    )

    assert captured["args"][:15] == tuple(tensors)
    assert captured["args"][15] == [16, 16, 16, 64, 128, 128]
    assert captured["args"][16] is flags
    assert len(captured["args"]) == 17


@pytest.mark.parametrize("has_finalize_schema", [False, True])
@pytest.mark.parametrize("has_demote_schema", [False, True])
def test_byte_v2_hybrid_raw_tail_requires_complete_schema(
    monkeypatch,
    has_finalize_schema,
    has_demote_schema,
):
    q1_argument_names = ["fuse_commit_finalize"] if has_finalize_schema else []
    q1_op = SimpleNamespace(
        default=SimpleNamespace(
            _schema=SimpleNamespace(
                arguments=[SimpleNamespace(name=name) for name in q1_argument_names]
            )
        )
    )
    multi_argument_names = ["demote_safe_raw_pages"] if has_demote_schema else []
    multi_token_op = SimpleNamespace(
        default=SimpleNamespace(
            _schema=SimpleNamespace(
                arguments=[SimpleNamespace(name=name) for name in multi_argument_names]
            )
        )
    )

    def fake_find_op(namespace, op_name):
        assert namespace == "_C_cache_ops"
        if op_name == "byte_v2_update_hybrid_cache_raw_tail_q1":
            return q1_op
        if op_name == "byte_v2_update_hybrid_cache_raw_staging_multi_token":
            return multi_token_op
        raise AssertionError(op_name)

    monkeypatch.setattr(byte_v2_ops_module, "_find_op", fake_find_op)

    assert byte_v2_ops_module.byte_v2_hybrid_raw_tail_q1_is_available() is (
        has_finalize_schema and has_demote_schema
    )
...
def test_byte_v2_hybrid_multi_token_update_wrapper_forwards_optional_flags(
    ...
    def fake_require_op(namespace, requested_op_name):
        assert namespace == "_C_cache_ops"
        assert (
            requested_op_name == "byte_v2_update_hybrid_cache_raw_staging_multi_token"
        )
...
def test_byte_v2_hybrid_multi_token_wrapper_omits_disabled_demote(monkeypatch):
    captured = {}
...
def test_byte_v2_hybrid_rejects_nondefault_tile_policy(monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="default ByteV2 tile policy"):
        byte_v2_attn_module.ByteV2AttentionImpl(
            num_heads=32,
            head_size=128,
            scale=0.125,
...
def test_byte_v2_kv_cache_spec_uses_byte_page_layout():
    ...
    assert spec.real_page_size_bytes == ByteV2PageLayoutV6().page_size_bytes
    assert spec.page_size_bytes == ByteV2PageLayoutV6().page_size_bytes
    assert (
        spec.page_metadata_size_bytes
        == ByteV2PageLayoutV6().aligned_metadata_bytes
        == 896
    )
    assert backend_cls.get_kv_cache_shape(
        num_blocks=3,
...
def test_page_layout_v6_parameterizes_outlier_pool_capacity():
    layout = ByteV2PageLayoutV6(outlier_pool_entries=2048)
...
def test_byte_v2_kv_cache_spec_rejects_uncompiled_layout_capacity():
    with pytest.raises(ValueError, match="compiled V6 CUDA layout"):
...
def test_byte_v2_ops_fail_cleanly_without_registered_kernels():
    ...
    value = torch.empty_like(key)
    kv_cache = torch.empty(
        (1, ByteV2PageLayoutV6().page_size_bytes),
        dtype=torch.uint8,
    )
...
def test_byte_v2_kv_cache_update_uses_and_reuses_raw_staging(monkeypatch):
    ...
    value = torch.empty_like(key)
    kv_cache = torch.empty((4, ByteV2PageLayoutV6().page_size_bytes), dtype=torch.uint8)
...
def test_byte_v2_raw_staging_manager_uses_fused_hybrid_q1_update(monkeypatch):
    ...
    result = manager._update_one_wave(
        ...
        kv_cache=torch.empty(
            (2, ByteV2PageLayoutV6().page_size_bytes), dtype=torch.uint8
        ),
        slot_mapping=slot_mapping,
...
def test_byte_v2_raw_staging_manager_hydrates_b1_cached_prefill(monkeypatch):
    ...
    kv_cache = torch.empty(
        (5, ByteV2PageLayoutV6().page_size_bytes),
        dtype=torch.uint8,
    )
...
def test_byte_v2_decoded_prefix_cache_hits_and_invalidates(monkeypatch):
    layout = ByteV2RawStagingLayout()
    hybrid_state = SimpleNamespace(
        raw_pages=torch.empty((2, layout.slot_size_bytes), dtype=torch.uint8),
        page_to_raw_slot=torch.tensor([-1, 1, -1, 0], dtype=torch.int32),
        fatal=torch.zeros((1,), dtype=torch.int32),
    )
    manager = byte_v2_attn_module.ByteV2RawStagingManager(
        tile_policy=DEFAULT_BYTE_V2_TILE_POLICY,
    ...
    manager.bind_decoded_prefix_cache(kv_cache, 2)
    calls = []
    ...
    first = manager.stage_cached_prefix(
        kv_cache=kv_cache,
        block_table=block_table,
        seq_len=32,
        prefix_cache_key=7,
    )
    second = manager.stage_cached_prefix(
        kv_cache=kv_cache,
        block_table=block_table,
        seq_len=32,
        prefix_cache_key=7,
    ...
    assert manager.decoded_prefix_cache_nbytes() == 2 * (65_536 + 8)
    assert len(calls) == 1
    hydrate_args = calls[0][0]
    assert torch.equal(hydrate_args[4], block_table[0])
    assert torch.equal(hydrate_args[5], torch.tensor([16, 16]))

    manager.invalidate_decoded_prefix_cache()
    third = manager.stage_cached_prefix(
        kv_cache=kv_cache,
        block_table=block_table,
        seq_len=32,
        prefix_cache_key=7,
    ...
    assert third is not None and third[1] is False
    assert len(calls) == 2


def test_byte_v2_decoded_prefix_cache_overflow_uses_transient_workspace(
    monkeypatch,
):
    layout = ByteV2RawStagingLayout()
    workspace = byte_v2_attn_module.ByteV2RawStagingWorkspaceSpec(
    ...
    monkeypatch.setattr(
        byte_v2_attn_module,
        "byte_v2_hydrate_raw_staging_from_hybrid_cache",
        lambda *args, **kwargs: transient_calls.append((args, kwargs)),
    )
    monkeypatch.setattr(
        byte_v2_attn_module,
        "byte_v2_static_w16_hydrate_raw_staging_from_hybrid_cache",
        lambda *args, **kwargs: pytest.fail("overflow used persistent cache"),
    )

    staged = manager.stage_cached_prefix(
        kv_cache=kv_cache,
    ...
    assert staged is not None and staged[1] is True
    assert staged[0][0].data_ptr() == workspace.raw_staging.data_ptr()
...
def test_byte_v2_raw_staging_manager_releases_failed_cached_hydrate(
    ...
    with pytest.raises(RuntimeError, match="hydrate failed"):
        manager.stage_cached_prefill(
            kv_cache=torch.empty(
                (2, ByteV2PageLayoutV6().page_size_bytes),
                dtype=torch.uint8,
            ),
...
def test_static_w16_batched_q1_bypasses_raw_staging(monkeypatch):
    monkeypatch.setenv("BYTE_V2_STATIC_W16_BATCHED_RAW_TAIL_Q1", "1")
    layout = ByteV2RawStagingLayout()
    hybrid_state = SimpleNamespace(
        raw_pages=torch.empty((4, layout.slot_size_bytes), dtype=torch.uint8),
        page_to_raw_slot=torch.full((8,), -1, dtype=torch.int32),
        free_slots=torch.arange(4, dtype=torch.int32),
        free_count=torch.tensor((4,), dtype=torch.int32),
        fatal=torch.zeros((1,), dtype=torch.int32),
    )
    manager = byte_v2_attn_module.ByteV2RawStagingManager(
        tile_policy=DEFAULT_BYTE_V2_TILE_POLICY,
    ...
    manager.configure_static_w16_bases((115, 110))
    calls = []

    def batched_q1(*args, **kwargs):
        calls.append((args, kwargs))
    ...
    key = torch.empty((4, 8, 128), dtype=torch.bfloat16)
    value = torch.empty_like(key)
    slot_mapping = SimpleNamespace(shape=(4,), is_cuda=True)
    metadata = SimpleNamespace(
        num_actual_tokens=4,
        max_query_len=1,
    ...
    assert result == (True, False)
    assert len(calls) == 1
    assert calls[0][0][0] is key
    assert calls[0][0][1] is value
    assert calls[0][0][4] is slot_mapping
    assert calls[0][1] == {"k_base": 115, "v_base": 110}
...
def test_byte_v2_raw_staging_manager_uses_fused_hybrid_multi_token_update(
    ...
    assert calls[0][0][5] is slot_mapping
    assert calls[0][1]["page_unsafe_flags"] is page_flags
    assert calls[0][1]["demote_safe_raw_pages"] is True
...
@pytest.mark.parametrize(
    (
        "initial_skip_enabled",
        "aligned_skip_enabled",
...
def test_static_w16_skips_hydrate_only_for_proven_overwrite(
    monkeypatch,
    initial_skip_enabled,
    aligned_skip_enabled,
    retain_initial_prefill,
    ...
    monkeypatch.setenv(
        ...
        "1" if initial_skip_enabled else "0",
    )
    monkeypatch.setenv(
        "BYTE_V2_STATIC_W16_PAGE_ALIGNED_PREFILL_SKIP_HYDRATE",
        "1" if aligned_skip_enabled else "0",
    )
    layout = ByteV2RawStagingLayout()
    workspace = byte_v2_attn_module.ByteV2RawStagingWorkspaceSpec(
        num_blocks=2,
    ...
    manager.configure_static_w16_bases((127, 127))
    manager.bind_shared_workspace(workspace)
    calls = []
    ...
    result = manager._update_one_wave(
        ...
        hybrid_state=hybrid_state,
        active_slot_capacity=2,
        retain_initial_prefill=retain_initial_prefill,
        overwrites_all_valid_rows=overwrites_all_valid_rows,
    )
    ...
    expected.extend(("append", "commit", "release"))
...
def test_byte_v2_attention_forward_zeros_output_for_profile_run(monkeypatch):
    ...
    key = torch.empty((1, 8, 128), dtype=torch.bfloat16)
    value = torch.empty_like(key)
    kv_cache = torch.empty((1, ByteV2PageLayoutV6().page_size_bytes), dtype=torch.uint8)
    output = torch.empty_like(query)
...
def test_byte_v2_attention_metadata_builder_preserves_common_prefix_len():
    ...
    assert metadata.is_prefilling is common_attn_metadata.is_prefilling
    assert metadata.cascade_prefix_cache_key == 91
    assert metadata.use_cascade is True
    assert torch.equal(metadata.cu_prefix_query_lens, torch.tensor([0, 2]))
    assert torch.equal(metadata.prefix_kv_lens, torch.tensor([48]))
    assert torch.equal(metadata.suffix_kv_lens, torch.tensor([2]))


def test_byte_v2_metadata_builder_delegates_static_w16_cascade_policy(
    monkeypatch,
):
    ...
    monkeypatch.setattr(flash_attn_module, "use_cascade_attention", select)
    monkeypatch.delenv("BYTE_V2_STATIC_W16_CASCADE_ATTENTION", raising=False)
    kwargs = {
        "common_prefix_len": 4096,
        "query_lens": torch.ones(8, dtype=torch.int32).numpy(),
        "num_query_heads": 32,
        "num_kv_heads": 8,
        "use_alibi": False,
    ...
    assert builder.use_cascade_attention(**kwargs) is True
    assert calls == [kwargs]

    monkeypatch.setenv("BYTE_V2_STATIC_W16_CASCADE_ATTENTION", "0")
    assert builder.use_cascade_attention(**kwargs) is False
    assert calls == [kwargs]


def test_gpu_attn_metadata_propagates_default_model_prefill_semantics():
    ...
    is_prefilling = torch.tensor([True, False, True], dtype=torch.bool)

    metadata_by_layer = build_attn_metadata(
        attn_groups=[[attn_group]],
        num_reqs=2,
        num_tokens=5,
        query_start_loc_gpu=torch.tensor([0, 3, 5], dtype=torch.int32),
        query_start_loc_cpu=torch.tensor([0, 3, 5], dtype=torch.int32),
        max_query_len=3,
        seq_lens=torch.tensor([19, 7], dtype=torch.int32),
        max_seq_len=19,
        ...
        seq_lens_cpu_upper_bound=torch.tensor([19, 7], dtype=torch.int32),
        is_prefilling=is_prefilling,
    )

    metadata = metadata_by_layer["model.layers.0.self_attn.attn"]
    assert torch.equal(metadata.is_prefilling, is_prefilling[:2])


def test_gpu_attn_metadata_propagates_selected_cascade_prefix():
    class CaptureBuilder:
        def build(self, common_prefix_len, common_attn_metadata):
            assert common_prefix_len == 4080
            return common_attn_metadata
    ...
    metadata_by_layer = build_attn_metadata(
        ...
        num_reqs=8,
        num_tokens=128,
        query_start_loc_gpu=torch.arange(0, 129, 16, dtype=torch.int32),
        query_start_loc_cpu=torch.arange(0, 129, 16, dtype=torch.int32),
        max_query_len=16,
        seq_lens=torch.full((8,), 4096, dtype=torch.int32),
        max_seq_len=4096,
        block_tables=(torch.zeros((8, 256), dtype=torch.int32),),
        slot_mappings=torch.zeros((1, 128), dtype=torch.int64),
        kv_cache_config=SimpleNamespace(kv_cache_groups=[object()]),
        cascade_attn_prefix_lens=[[4080]],
        cascade_prefix_cache_key=37,
    )

    metadata = metadata_by_layer["model.layers.0.self_attn.attn"]
    assert metadata.cascade_prefix_cache_key == 37


def test_gpu_v2_selects_page_aligned_cascade_prefix(monkeypatch):
    calls = []
    ...
    monkeypatch.setattr(
        "vllm.v1.worker.gpu.attn_utils.num_compute_units",
        lambda _: 84,
    )

    result = compute_cascade_attn_prefix_lens(
        [[attn_group]],
        np.full(8, 16, dtype=np.int32),
        np.full(8, 4087, dtype=np.int32),
        [300],
    )

    assert result == [[4080]]
    assert len(calls) == 1
    query_lens = calls[0].pop("query_lens")
    assert np.array_equal(query_lens, np.full(8, 16, dtype=np.int32))
    assert calls[0] == {
        "common_prefix_len": 4080,
        "num_query_heads": 32,
        "num_kv_heads": 8,
        "use_alibi": False,
        "use_sliding_window": False,
...
def test_byte_v2_direct_prefill_routes_cached_prefix_and_initial_groups(monkeypatch):
    ...
    output = torch.empty_like(query)
    kv_cache = torch.empty((8, 50_560), dtype=torch.uint8)
    metadata = SimpleNamespace(
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="requires CUDA")
@pytest.mark.parametrize("num_splits", [1, 4], ids=["nonsplit", "split4"])
def test_byte_v2_fa2_safe_pages_ignore_cold_head_metadata(num_splits):
    if not _has_torch_op("_C_cache_ops", "byte_v2_reshape_and_cache"):
        pytest.skip("ByteV2 cache custom op is not registered")
    ...
    tensors = make_inputs((129,), query_len=1, force_outliers=False)
    byte_cache = tensors["byte_cache"]
    assert isinstance(byte_cache, torch.Tensor)
    layout = ByteV2PageLayoutV6()
    for physical_page in range(byte_cache.size(0)):
        for kv_head in range(layout.num_kv_heads):
            assert (
                _load_u32_bytes(
                    byte_cache,
            ...
            cold_begin = layout.kv_head_meta_offset(kv_head=kv_head) + 32
            cold_end = layout.kv_head_meta_offset(kv_head=kv_head) + 96
            byte_cache[physical_page, cold_begin:cold_end].fill_(0xFF)
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="requires CUDA")
@pytest.mark.parametrize(
    ("seq_len", "force_outliers", "expects_split20"),
    [
        pytest.param(32767, False, False, id="below-lower-bound"),
        pytest.param(32768, False, True, id="lower-bound"),
        pytest.param(65536, False, True, id="safe-midrange"),
        pytest.param(65536, True, True, id="outlier-midrange"),
        pytest.param(131072, False, True, id="upper-bound"),
        pytest.param(131073, False, False, id="above-upper-bound"),
    ],
)
def test_byte_v2_fa2_a40_long_q1_auto_split20_boundaries(
    seq_len,
    force_outliers,
    expects_split20,
):
    ...
    if properties.name != "NVIDIA A40" or properties.multi_processor_count != 84:
        pytest.skip("requires the validated 84-SM NVIDIA A40 split heuristic")
    ...
    add_hybrid_raw_pages(tensors, (), poison_compact_pages=False)
    query = tensors["query"]
    ...
    def run(backend, num_splits):
        ...
        return torch.ops._vllm_fa2_C.byte_v2_hybrid_varlen_fwd(
            query,
            tensors["byte_cache"],
            tensors["raw_staging"],
    ...
    results = {
        (backend, num_splits): run(backend, num_splits)
        for backend in ("raw", "hybrid")
        for num_splits in (0, 18, 20)
    }

    def is_bitwise_equal(lhs, rhs):
        lhs_out, lhs_lse = lhs
        rhs_out, rhs_lse = rhs
        return torch.equal(
            lhs_out.view(torch.int16),
            rhs_out.view(torch.int16),
        ) and torch.equal(
            lhs_lse.view(torch.int32),
            rhs_lse.view(torch.int32),
        )

    for num_splits in (0, 18, 20):
        assert is_bitwise_equal(
            results[("raw", num_splits)],
            results[("hybrid", num_splits)],
        )

    # Explicit positive split counts must bypass the auto selector. The
    # deterministic oracle distinguishes the two reduction trees on every
    # parameterized shape.
    assert not is_bitwise_equal(results[("raw", 18)], results[("raw", 20)])
    expected_split = 20 if expects_split20 else 18
    for backend in ("raw", "hybrid"):
        assert is_bitwise_equal(
            results[(backend, 0)],
            results[(backend, expected_split)],
        )


@pytest.mark.skipif(not torch.cuda.is_available(), reason="requires CUDA")
@pytest.mark.parametrize(
    ("num_query_heads", "softcap"),
    [
        pytest.param(16, 0.0, id="gqa2"),
        pytest.param(32, 10.0, id="softcap"),
    ],
)
def test_byte_v2_fa2_a40_long_q1_auto_split20_near_misses(
    num_query_heads,
    softcap,
):
    properties = torch.cuda.get_device_properties(
    ...
    query = tensors["query"]
    assert isinstance(query, torch.Tensor)
    query = query[:, :num_query_heads].contiguous()
    ...
    results = {num_splits: run(num_splits) for num_splits in (0, 18, 20)}
    ...
    assert is_bitwise_equal(results[0], results[18])
    assert not is_bitwise_equal(results[18], results[20])
...
def test_byte_v2_cached_prefill_full_hydrate_matches_raw_fa2_bitwise(source):
    ...
    if source == "compact-outlier":
        ...
        overflow_offset = ByteV2PageLayoutV6().outlier_pool_overflow_offset
...
from vllm.v1.attention.backends.byte_v2_layout import ByteV2PageLayoutV6
...
def _byte_v2_fused_hybrid_writer_multi_token_update(
    ...
    *,
    ...
    demotion_kwargs = {} if retain else {"demote_safe_raw_pages": demote_safe_raw_pages}
    update_op(
        key,
        value,
...
def _byte_v2_raw_tail_q1_update(
    ...
    *,
    ...
    update_op(
        ...
        tile_policy=(16, 16, 16, 64, 128, 128),
        page_unsafe_flags=page_unsafe_flags,
        fuse_commit_finalize=fuse_commit_finalize,
    )
...
def _run_byte_v2_parallel_prepare_selector_subprocess(env_value: str | None):
    probe_source = r"""
import runpy
import sys

namespace = runpy.run_path(sys.argv[1])
namespace["_byte_v2_cooperative_writer_n106_capacity33_probe"]()
"""
    env = os.environ.copy()
    env["BYTE_V2_HYBRID_COOPERATIVE_WRITER_LIMIT"] = "144"
    if env_value is None:
        env.pop("BYTE_V2_HYBRID_PARALLEL_PREPARE", None)
    else:
        env["BYTE_V2_HYBRID_PARALLEL_PREPARE"] = env_value
    return subprocess.run(
        [
            sys.executable,
            "-c",
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="requires CUDA")
@pytest.mark.parametrize("env_value", [None, "0", "1"])
def test_byte_v2_parallel_prepare_selector_is_canonical_subprocess(env_value):
    if not _byte_v2_fused_hybrid_writer_multi_token_op_is_available():
        pytest.skip("ByteV2 fused hybrid multi-token writer op is not registered")

    completed = _run_byte_v2_parallel_prepare_selector_subprocess(env_value)
    combined_output = completed.stdout + completed.stderr
    assert completed.returncode == 0, combined_output
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="requires CUDA")
def test_byte_v2_parallel_prepare_invalid_env_fails_closed_subprocess():
    if not _byte_v2_fused_hybrid_writer_multi_token_op_is_available():
        pytest.skip("ByteV2 fused hybrid multi-token writer op is not registered")

    completed = _run_byte_v2_parallel_prepare_selector_subprocess("2")
    combined_output = completed.stdout + completed.stderr
    assert completed.returncode != 0, combined_output
    assert "BYTE_V2_HYBRID_PARALLEL_PREPARE must be unset, 0, or 1, got 2" in (
        combined_output
    )
    assert "cooperative_writer_n106_capacity33_ok" not in combined_output
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="requires CUDA")
@pytest.mark.parametrize("demote_safe_raw_pages", [False, True])
def test_byte_v2_fused_hybrid_multi_token_all_dummy_q34_is_noop(
    ...
    _byte_v2_fused_hybrid_writer_multi_token_update(
        ...
        active_capacity=2,
        page_unsafe_flags=page_flags,
        demote_safe_raw_pages=demote_safe_raw_pages,
    )
    torch.accelerator.synchronize()
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="requires CUDA")
@pytest.mark.parametrize("fuse_commit_finalize", [False, True])
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="requires CUDA")
@pytest.mark.parametrize("force_outliers", [False, True])
def test_byte_v2_raw_tail_q1_q_gt_1_demotes_safe_full_page(force_outliers):
    if not _byte_v2_raw_tail_q1_op_is_available():
        pytest.skip("ByteV2 raw-tail Q1 writer op is not registered")
    if not _byte_v2_fused_hybrid_writer_multi_token_op_is_available():
        pytest.skip("ByteV2 fused hybrid writer op is not registered")
    ...
    assert int(tensors["page_to_raw_slot"][0].item()) == -1
    assert int(state.free_raw_slot_count.item()) == 1
    assert int(state.free_raw_slots[0].item()) == raw_slot
    assert int(state.raw_pool_overflow.item()) == 0
    if force_outliers:
        stats = collect_byte_v2_fallback_stats(
            tensors["byte_cache"].cpu(),
            layout=ByteV2PageLayoutV6(),
        )
        assert stats.overlay_tiles > 0
        assert stats.fallback_tiles == 0
        assert int(page_flags[0].item()) != 0
    _assert_byte_v2_hybrid_writer_matches_raw(tensors)
    _assert_byte_v2_hybrid_writer_transient_state_is_clean(state)
    ...
    _byte_v2_hybrid_writer_reset(
        tensors,
        state,
        torch.tensor([0], dtype=torch.int32, device=page_flags.device),
    )
    torch.accelerator.synchronize()
    assert int(state.free_raw_slot_count.item()) == 1
    assert int(state.raw_pool_overflow.item()) == 0
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="requires CUDA")
def test_byte_v2_raw_tail_q1_q_gt_1_keeps_partial_tail():
    if not _byte_v2_raw_tail_q1_op_is_available():
        pytest.skip("ByteV2 raw-tail Q1 writer op is not registered")
    if not _byte_v2_fused_hybrid_writer_multi_token_op_is_available():
        pytest.skip("ByteV2 fused hybrid writer op is not registered")
    ...
    assert int(tensors["page_to_raw_slot"][0].item()) == raw_slot
    assert int(state.free_raw_slot_count.item()) == 0
    assert int(state.raw_pool_overflow.item()) == 0
    _assert_byte_v2_hybrid_writer_transient_state_is_clean(state)

    # Returning to Q1 must append to the same persistent tail instead of
    # allocating a replacement sidecar slot.
    _byte_v2_raw_tail_q1_update(
        tensors,
        state,
        slot_mapping[3:],
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="requires CUDA")
def test_byte_v2_raw_tail_q1_q_gt_1_keeps_unsafe_raw_authority_bitwise():
    if not _byte_v2_raw_tail_q1_op_is_available():
        pytest.skip("ByteV2 raw-tail Q1 writer op is not registered")
    if not _byte_v2_fused_hybrid_writer_multi_token_op_is_available():
        pytest.skip("ByteV2 fused hybrid writer op is not registered")
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="requires CUDA")
def test_byte_v2_q_gt_1_reuses_demoted_slot_for_new_unsafe_page_bitwise():
    if not _byte_v2_raw_tail_q1_op_is_available():
        pytest.skip("ByteV2 raw-tail Q1 writer op is not registered")
    if not _byte_v2_fused_hybrid_writer_multi_token_op_is_available():
        pytest.skip("ByteV2 fused hybrid writer op is not registered")
    ...
    unsafe_tensors, _, _ = _make_byte_v2_hybrid_writer_case(
        seq_len=16,
        force_pool_overflow=True,
        raw_pool_slots=1,
    )
    tensors["key"][1].copy_(unsafe_tensors["key"][0])
    tensors["value"][1].copy_(unsafe_tensors["value"][0])
    ...
    # Demotion returns page 0's safe-full slot in a standalone kernel. The
    # following persist kernel may then allocate it to unsafe page 1.
    _byte_v2_fused_hybrid_writer_multi_token_update(
        tensors,
        state,
        slot_mapping[1:],
    ...
    assert tensors["page_to_raw_slot"].cpu().tolist() == [-1, raw_slot]
    assert int(state.free_raw_slot_count.item()) == 0
    assert int(state.raw_pool_overflow.item()) == 0
    _assert_byte_v2_hybrid_writer_matches_raw(tensors)
    _assert_byte_v2_hybrid_writer_transient_state_is_clean(state)
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="requires CUDA")
def test_byte_v2_q_gt_1_demotes_multiple_safe_full_pages():
    if not _byte_v2_raw_tail_q1_op_is_available():
        pytest.skip("ByteV2 raw-tail Q1 writer op is not registered")
    if not _byte_v2_fused_hybrid_writer_multi_token_op_is_available():
        pytest.skip("ByteV2 fused hybrid writer op is not registered")
    ...
    torch.accelerator.synchronize()
    raw_slots = tensors["page_to_raw_slot"].cpu().tolist()
    assert sorted(raw_slots) == [0, 1]
    assert int(state.free_raw_slot_count.item()) == 0

    remaining_slots = torch.cat((slot_mapping[1:16], slot_mapping[17:]))
    _byte_v2_fused_hybrid_writer_multi_token_update(
        tensors,
        state,
        remaining_slots,
    ...
    assert int(state.free_raw_slot_count.item()) == 2
    assert sorted(state.free_raw_slots[:2].cpu().tolist()) == raw_slots
    assert int(state.raw_pool_overflow.item()) == 0
    _assert_byte_v2_hybrid_writer_matches_raw(tensors)
    _assert_byte_v2_hybrid_writer_transient_state_is_clean(state)
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="requires CUDA")
def test_byte_v2_q_gt_1_demote_cuda_graph_reuses_raw_tail_slot():
    if not _byte_v2_raw_tail_q1_op_is_available():
        pytest.skip("ByteV2 raw-tail Q1 writer op is not registered")
    if not _byte_v2_fused_hybrid_writer_multi_token_op_is_available():
        pytest.skip("ByteV2 fused hybrid writer op is not registered")
    ...
    flat_key = tensors["key"].view(-1, 8, 128)
    flat_value = tensors["value"].view(-1, 8, 128)
    graph_key = torch.zeros_like(flat_key[:15])
    graph_value = torch.zeros_like(flat_value[:15])
    graph_slots = torch.full(
        (15,),
        -1,
        dtype=torch.int64,
        device=slot_mapping.device,
    ...
    def replay(start: int) -> None:
        graph_key.copy_(flat_key[start : start + 15])
        graph_value.copy_(flat_value[start : start + 15])
        graph_slots.copy_(slot_mapping[start : start + 15])
        graph.replay()
        torch.accelerator.synchronize()
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="requires CUDA")
@pytest.mark.parametrize("fuse_commit_finalize", [False, True])
def test_byte_v2_raw_tail_q1_dummy_slot_is_noop(fuse_commit_finalize):
    if not _byte_v2_raw_tail_q1_op_is_available():
        pytest.skip("ByteV2 raw-tail Q1 writer op is not registered")
...
def _byte_v2_fused_hybrid_multi_token_fatal_probe(probe: str) -> None:
    ...
    if probe == "invalid-map":
        ...
        active_capacity = 31
    elif probe == "max-boundary-duplicate-slot":
        tensors, state, _ = _make_byte_v2_hybrid_writer_case(
            seq_len=144 * 16, raw_pool_slots=144
        )
        slot_mapping = (
            torch.arange(
                144,
        ...
        slot_mapping[-1] = slot_mapping[0]
        active_capacity = 144
    elif probe == "dirty-staging-descriptor":
        tensors, state, _ = _make_byte_v2_hybrid_writer_case(seq_len=2)
        state.staging_to_physical_block[0] = 0
        slot_mapping = torch.tensor(
            [0, 1], dtype=torch.int64, device=tensors["byte_cache"].device
        )
        active_capacity = 1
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "max-boundary-duplicate-slot",
        "dirty-staging-descriptor",
        "pool-exhaustion",
    ],
)
...
"""
    env = os.environ.copy()
    env["BYTE_V2_HYBRID_COOPERATIVE_WRITER_LIMIT"] = "144"
    env["BYTE_V2_HYBRID_PARALLEL_PREPARE"] = "1"
    completed = subprocess.run(
        [
            sys.executable,
            "-c",
...
def test_byte_v2_fa2_fatal_v6_page_fails_closed_in_subprocess(fatal_option):
...
def _byte_v2_legacy_fallback_without_raw_probe() -> None:
    assert _has_torch_op("_C", "byte_v2_paged_decode_attention")
    layout = ByteV2PageLayoutV6()
    ...
    query = torch.zeros((1, 32, 128), dtype=torch.bfloat16, device="cuda")
    output = torch.empty_like(query)
    block_tables = torch.zeros((1, 1), dtype=torch.int32, device="cuda")
    seq_lens = torch.ones((1,), dtype=torch.int32, device="cuda")
    torch.accelerator.synchronize()
    print({"legacy_compact_fallback_probe": True}, flush=True)
    byte_v2_paged_decode_attention(
        output,
        query,
        kv_cache,
    ...
    torch.accelerator.synchronize()
    print({"legacy_compact_fallback_unexpected_success": True}, flush=True)


@pytest.mark.skipif(not torch.cuda.is_available(), reason="requires CUDA")
def test_byte_v2_legacy_compact_fallback_fails_closed_subprocess():
...
namespace = runpy.run_path(sys.argv[1])
namespace["_byte_v2_legacy_fallback_without_raw_probe"]()
"""
    completed = subprocess.run(
        [sys.executable, "-c", probe_source, str(Path(__file__).resolve())],
        cwd=Path(__file__).resolve().parents[3],
    ...
    combined_output = completed.stdout + completed.stderr
    assert completed.returncode != 0, combined_output
    assert "legacy_compact_fallback_probe" in combined_output
    assert "legacy_compact_fallback_unexpected_success" not in combined_output
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="requires CUDA")
def test_byte_v2_reshape_and_cache_cuda_writes_v6_payload():
    if not _has_torch_op("_C_cache_ops", "byte_v2_reshape_and_cache"):
...
def test_byte_v2_reshape_and_cache_cuda_writes_outlier_overlay():
    ...
    k_entry = _load_u16_bytes(
        cache,
        0,
        _compact_outlier_payload_offset(
            cache,
            layout,
            physical_block=0,
            kv_head=0,
...
def test_byte_v2_reshape_and_cache_sideband_high_cuda_writes_outlier_sideband():
    ...
    # The diagnostic sideband format reserves a full 256-byte high plane for
    # every overlay tile, so it intentionally retains the V5 1024-entry pool.
    layout = ByteV2PageLayoutV5(
    ...
    assert k_outlier_mask & 1
    assert int(cache[0, layout.k_outlier_count_offset(kv_head=0, dim_tile=0)]) == 1
    k_sideband_offset = _compact_outlier_payload_offset(
        cache,
        layout,
        physical_block=0,
        kv_head=0,
    ...
    assert v_outlier_mask & 1
    assert int(cache[0, layout.v_outlier_count_offset(kv_head=0, dim_tile=0)]) == 1
    v_sideband_offset = _compact_outlier_payload_offset(
        cache,
        layout,
        physical_block=0,
        kv_head=0,
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="requires CUDA")
def test_byte_v2_sideband_high_decode_rejects_v6_page_size():
    ...
    query = torch.zeros((1, 32, 128), dtype=torch.bfloat16, device="cuda")
    output = torch.empty_like(query)
    exp_sums = torch.empty((1, 32, 1), dtype=torch.float32, device="cuda")
    max_logits = torch.empty((0,), dtype=torch.float32, device="cuda")
    tmp_out = torch.empty((1, 32, 1, 128), dtype=torch.float32, device="cuda")
    kv_cache = torch.zeros((1, 50_560), dtype=torch.uint8, device="cuda")
    page_unsafe_flags = torch.zeros((1,), dtype=torch.int32, device="cuda")
    block_tables = torch.zeros((1, 1), dtype=torch.int32, device="cuda")
    seq_lens = torch.ones((1,), dtype=torch.int32, device="cuda")

    with pytest.raises(RuntimeError, match="sideband-high layout"):
        byte_v2_paged_decode_attention_split_k_guarded(
            output,
            exp_sums,
            max_logits,
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="requires CUDA")
def test_byte_v2_reshape_and_cache_cuda_block_direct_writes_v6_payload():
    if not _has_torch_op("_C_cache_ops", "byte_v2_reshape_and_cache"):
        pytest.skip("ByteV2 custom ops are not registered")
...
def _byte_v2_raw_staging_prepare_oracle(
    slot_mapping: list[int],
    *,
    num_physical_blocks: int,
    num_staging_slots: int,
) -> tuple[list[int], list[int], list[int], int, int]:
    """Reproduce the public prepare metadata semantics on the CPU."""
    block_to_staging_slot = [-1] * num_physical_blocks
    staging_to_physical_block = [-1] * num_staging_slots
    valid_rows = [0] * num_staging_slots
    next_staging_slot = 0
    overflow = 0
    for slot_idx in slot_mapping:
        if slot_idx < 0:
            continue
        physical_block, row = divmod(slot_idx, 16)
        if physical_block >= num_physical_blocks:
            overflow = 1
            continue
        staging_slot = block_to_staging_slot[physical_block]
        if staging_slot < 0:
            staging_slot = next_staging_slot
            next_staging_slot += 1
        ...
        valid_rows[staging_slot] = max(valid_rows[staging_slot], row + 1)
    return (
        block_to_staging_slot,
        staging_to_physical_block,
        valid_rows,
...
def _make_byte_v2_raw_staging_prepare_state(
    *,
    num_physical_blocks: int,
    num_staging_slots: int,
    ...
    return (
        torch.full((num_physical_blocks,), -1, dtype=torch.int32, device=device),
        torch.full((num_staging_slots,), -1, dtype=torch.int32, device=device),
        torch.zeros((num_staging_slots,), dtype=torch.int32, device=device),
        torch.zeros((1,), dtype=torch.int32, device=device),
        torch.full((1,), 7, dtype=torch.int32, device=device),
    )


def _assert_byte_v2_raw_staging_prepare_metadata(
    state: tuple[
        torch.Tensor,
        torch.Tensor,
        torch.Tensor,
    ...
    expected: tuple[list[int], list[int], list[int], int, int],
) -> None:
    actual = tuple(tensor.cpu().tolist() for tensor in state[:3])
    assert actual == expected[:3]
    assert int(state[3].cpu().item()) == expected[3]
    assert int(state[4].cpu().item()) == expected[4]


def _release_and_assert_byte_v2_raw_staging_prepare_state_is_clean(
    state: tuple[
        torch.Tensor,
        torch.Tensor,
        torch.Tensor,
    ...
    byte_v2_release_raw_staging(*state)
    torch.accelerator.synchronize()
    block_to_slot, slot_to_block, valid_rows, next_slot, overflow = state
    assert block_to_slot.cpu().tolist() == [-1] * block_to_slot.numel()
    assert slot_to_block.cpu().tolist() == [-1] * slot_to_block.numel()
    assert valid_rows.cpu().tolist() == [0] * valid_rows.numel()
    assert int(next_slot.cpu().item()) == 0
    assert int(overflow.cpu().item()) == 0


@pytest.mark.skipif(not torch.cuda.is_available(), reason="requires CUDA")
@pytest.mark.parametrize("num_tokens", [255, 256, 257, 4096])
def test_byte_v2_raw_staging_prepare_contiguous_group_boundaries(num_tokens):
    ...
    num_pages = (num_tokens + 15) // 16
    expected = _byte_v2_raw_staging_prepare_oracle(
        slots,
        num_physical_blocks=num_pages,
        num_staging_slots=num_pages,
    )
    state = _make_byte_v2_raw_staging_prepare_state(
        num_physical_blocks=num_pages,
        num_staging_slots=num_pages,
    )
    slot_mapping = torch.tensor(slots, dtype=torch.int64, device="cuda")

    # Repeat to make deterministic publication and release/reuse part of the
    # fast-path contract, rather than accepting a one-run atomic coincidence.
    for _ in range(3):
        state[4].fill_(7)
        byte_v2_prepare_raw_staging(
            slot_mapping,
        ...
        torch.accelerator.synchronize()
        _assert_byte_v2_raw_staging_prepare_metadata(state, expected)
        _release_and_assert_byte_v2_raw_staging_prepare_state_is_clean(state)


_BYTE_V2_RAW_STAGING_PREPARE_FALLBACK_CASES = {
    "ragged": (
        [7 * 16]
        + [2 * 16 + row for row in range(15)]
        + [11 * 16 + row for row in range(16)]
        + [4 * 16 + row for row in range(16)]
        + [9 * 16],
        12,
    ),
    "permuted": (
        [5 * 16 + 15, 2 * 16 + 3, 5 * 16, 9 * 16 + 7, 2 * 16 + 14],
        10,
    ),
    "repeated-page": (
        [4 * 16 + 7, 4 * 16 + 2, 4 * 16 + 7, 16 + 15, 4 * 16 + 12, 16],
        5,
    ),
    "negative": ([-1, 3 * 16 + 4, -7, 16 + 15, -1, 3 * 16 + 2], 4),
    # Keep the first fifteen 16-token groups valid so these cases exercise
    # large-input validation cleanup before falling back to the serial path.
    "large-cross-page-group": (
        list(range(15 * 16)) + list(range(15 * 16 + 1, 16 * 16 + 1)),
        17,
    ),
    "large-nonadjacent-repeat": (
        list(range(15 * 16)) + [3 * 16 + row for row in range(16)],
        15,
    ),
    "large-negative": (
        list(range(15 * 16)) + [15 * 16 + row if row != 7 else -1 for row in range(16)],
        16,
    ),
    "large-out-of-range": (
        list(range(255)) + [16 * 16],
        16,
    ),
}
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="requires CUDA")
@pytest.mark.parametrize("case_name", _BYTE_V2_RAW_STAGING_PREPARE_FALLBACK_CASES)
def test_byte_v2_raw_staging_prepare_general_mapping_fallback(case_name):
    if not _has_torch_op("_C_cache_ops", "byte_v2_prepare_raw_staging"):
        pytest.skip("ByteV2 raw staging prepare custom op is not registered")
    if not _has_torch_op("_C_cache_ops", "byte_v2_release_raw_staging"):
        pytest.skip("ByteV2 raw staging release custom op is not registered")

    slots, num_physical_blocks = _BYTE_V2_RAW_STAGING_PREPARE_FALLBACK_CASES[case_name]
    unique_pages = len({slot // 16 for slot in slots if slot >= 0})
    num_staging_slots = unique_pages + 2
    expected = _byte_v2_raw_staging_prepare_oracle(
        slots,
    ...
    byte_v2_prepare_raw_staging(
        torch.tensor(slots, dtype=torch.int64, device="cuda"),
        *state,
        alloc_block_tokens=16,
    )
    torch.accelerator.synchronize()
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="requires CUDA")
def test_byte_v2_raw_staging_prepare_large_input_appends_nonempty_state():
    if not _has_torch_op("_C_cache_ops", "byte_v2_prepare_raw_staging"):
        pytest.skip("ByteV2 raw staging prepare custom op is not registered")
    if not _has_torch_op("_C_cache_ops", "byte_v2_release_raw_staging"):
        pytest.skip("ByteV2 raw staging release custom op is not registered")

    first_slots = [5 * 16 + 3, 2 * 16 + 7]
    second_pages = [2, *range(6, 21)]
    second_slots = [
        physical_block * 16 + row
        for physical_block in second_pages
        for row in range(16)
    ]
    assert len(second_slots) == 256
    num_physical_blocks = 21
    num_staging_slots = 19
    state = _make_byte_v2_raw_staging_prepare_state(
        num_physical_blocks=num_physical_blocks,
        num_staging_slots=num_staging_slots,
    )
    ...
    byte_v2_prepare_raw_staging(
        torch.tensor(first_slots, dtype=torch.int64, device="cuda"),
        *state,
        alloc_block_tokens=16,
    )
    torch.accelerator.synchronize()
    first_expected = _byte_v2_raw_staging_prepare_oracle(
        first_slots,
        num_physical_blocks=num_physical_blocks,
        num_staging_slots=num_staging_slots,
    )
    _assert_byte_v2_raw_staging_prepare_metadata(state, first_expected)
    assert int(state[3].cpu().item()) == 2

    # The second mapping has the large aligned shape accepted by the candidate
    # validator, but the nonzero cursor must force serial append/reuse instead
    # of overwriting the live descriptors from the first call.
    byte_v2_prepare_raw_staging(
        torch.tensor(second_slots, dtype=torch.int64, device="cuda"),
        *state,
        alloc_block_tokens=16,
    )
    torch.accelerator.synchronize()
    combined_expected = _byte_v2_raw_staging_prepare_oracle(
        first_slots + second_slots,
        num_physical_blocks=num_physical_blocks,
        num_staging_slots=num_staging_slots,
    )
    _assert_byte_v2_raw_staging_prepare_metadata(state, combined_expected)
    _release_and_assert_byte_v2_raw_staging_prepare_state_is_clean(state)


@pytest.mark.skipif(not torch.cuda.is_available(), reason="requires CUDA")
def test_byte_v2_raw_staging_prepare_cuda_graph_switches_device_paths():
    if not _has_torch_op("_C_cache_ops", "byte_v2_prepare_raw_staging"):
        pytest.skip("ByteV2 raw staging prepare custom op is not registered")
    if not _has_torch_op("_C_cache_ops", "byte_v2_release_raw_staging"):
        pytest.skip("ByteV2 raw staging release custom op is not registered")

    fast_slots = list(range(256))
    large_negative_slots = [*range(255), -1]
    cross_page_slots = list(range(15 * 16)) + list(range(15 * 16 + 1, 16 * 16 + 1))
    num_physical_blocks = 17
    num_staging_slots = 17
    state = _make_byte_v2_raw_staging_prepare_state(
        num_physical_blocks=num_physical_blocks,
        num_staging_slots=num_staging_slots,
    )
    slot_mapping = torch.tensor(fast_slots, dtype=torch.int64, device="cuda")
    ...
    def assert_prepared(slots: list[int]) -> None:
        torch.accelerator.synchronize()
        expected = _byte_v2_raw_staging_prepare_oracle(
            slots,
            num_physical_blocks=num_physical_blocks,
    ...
    # Warm dispatcher state before capture, then capture only prepare so the
    # external assertions and releases remain observable between replays.
    prepare()
    assert_prepared(fast_slots)
    graph = torch.cuda.CUDAGraph()
    with torch.cuda.graph(graph):
        prepare()
    graph.replay()
    ...
    for fallback_slots in (large_negative_slots, cross_page_slots):
        slot_mapping.copy_(
            torch.tensor(fallback_slots, dtype=torch.int64, device="cuda")
        )
        graph.replay()
        assert_prepared(fallback_slots)

    slot_mapping.copy_(torch.tensor(fast_slots, dtype=torch.int64, device="cuda"))
    graph.replay()
    assert_prepared(fast_slots)


@pytest.mark.skipif(not torch.cuda.is_available(), reason="requires CUDA")
def test_byte_v2_raw_staging_prepare_capacity_bypass_matches_serial():
    if not _has_torch_op("_C_cache_ops", "byte_v2_prepare_raw_staging"):
        pytest.skip("ByteV2 raw staging prepare custom op is not registered")
    if not _has_torch_op("_C_cache_ops", "byte_v2_release_raw_staging"):
        pytest.skip("ByteV2 raw staging release custom op is not registered")

    # Keep N above the parallel dispatch threshold while making the number of
    # page groups exceed capacity. The host must select the exact serial path,
    # preserve its overflow/cursor semantics, and leave no internal claim.
    slots = list(range(17 * 16))
    slot_mapping = torch.tensor(slots, dtype=torch.int64, device="cuda")
    state = _make_byte_v2_raw_staging_prepare_state(
        num_physical_blocks=18,
        num_staging_slots=16,
    )
    byte_v2_prepare_raw_staging(
        slot_mapping,
        *state,
    ...
    block_to_slot, slot_to_block, valid_rows, next_slot, overflow = state
    assert int(overflow.cpu().item()) == 1
    assert int(next_slot.cpu().item()) >= slot_to_block.numel()
    block_to_slot_cpu = block_to_slot.cpu().tolist()
    slot_to_block_cpu = slot_to_block.cpu().tolist()
    valid_rows_cpu = valid_rows.cpu().tolist()
    assert all(staging_slot >= -1 for staging_slot in block_to_slot_cpu)
    for staging_slot, physical_block in enumerate(slot_to_block_cpu):
        if physical_block < 0:
            assert valid_rows_cpu[staging_slot] == 0
            continue
        assert 0 <= physical_block < block_to_slot.numel()
        assert block_to_slot_cpu[physical_block] == staging_slot
        assert 1 <= valid_rows_cpu[staging_slot] <= 16
    _release_and_assert_byte_v2_raw_staging_prepare_state_is_clean(state)
...
def test_byte_v2_raw_staging_commit_matches_direct_cache(num_tokens):
    ...
    compressed_layout = ByteV2PageLayoutV6()
